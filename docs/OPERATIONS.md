...
빌드와 배포는 GitHub Actions가 수행합니다. 운영자는 **브랜치 병합, 버전 갱신, 변경 이력 작성과 푸시**를 진행합니다.
...
**실서비스 버전은 배포 주소의 응답으로 확인합니다.** 그래서 `build.json` 을 직접 읽습니다. `deploy ← main` 줄에 숫자가 있으면 **머지는 됐는데 실서비스에 안 나간 변경**이 그만큼 있다는 뜻입니다. 배포가 도중에 막혔을 때 이 줄에서 바로 보입니다.

### 무엇을 고쳤는지에 맞춰 빌드한다

전체 빌드는 원본 데이터와 스프라이트를 다시 생성합니다. 화면만 수정한 경우에는 화면 빌드만 실행하세요.
...
건너뛴 단계가 기대는 산출물이 없으면 **실패로 처리합니다.** 불완전한 `dist` 가 배포되는 것을 방지하기 위해서입니다. `--meta-only` 산출물이 전체 빌드와 바이트 단위로 같은 것은 확인했습니다(같은 채널 기준).
...
v4.0.1 에 터진 버그는 규칙이 아니라 **컴포넌트에서 규칙을 적용하지 않은** 배선 문제였습니다. 그래서 규칙만이 아니라 규칙을 쓰는 컴포넌트(메뉴 줄·홈 타일·잠금 카드)를 실제로 그려 봅니다. 그 버그를 코드에 다시 넣으면 21건 중 4건이 빨개지는 것을 확인했습니다.
...
**페이지뷰는 시행일 문 없이 바로 센다** (v5.2.1, 주인 결정). 방침(`Legal.tsx`) · `legalMeta.ts` 의 날짜가 9/24 이고,
