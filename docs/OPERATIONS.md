...
## 15. 인기 검색어 (2026-09-20 v4.5.5)

홈의 '오늘 많이 찾은 포켓몬' 은 GA4 에 쌓인 **완성어**를 하루 두 번 걷어 온 것이다.
`backend/hotsearch_build.py` 가 Data API 를 불러 `data/hotsearch.json` 을 쓰고, 배포가 그 파일을 함께 올린다.

### 사람이 할 것 — 최초 1회 (10분)

**시크릿 둘을 넣기 전까지 화면에는 이 구역이 안 나온다.** 빈 표를 쓰고 넘어가도록 되어 있어 배포는 멀쩡히 된다.

1. **GA4 속성 번호 확인** — GA 관리 > 속성 설정. 측정 ID(`G-...`)가 **아니라** 숫자 아홉 자리다.
2. **서비스 계정 만들기** — Google Cloud > IAM > 서비스 계정 > 키 만들기(JSON). 역할은 주지 않아도 된다.
3. **GA 속성에 뷰어로 추가** — GA 관리 > 속성 액세스 관리 > `+` > 그 서비스 계정 이메일, 역할 **뷰어**.
4. **Data API 켜기** — Google Cloud > API 및 서비스 > `Google Analytics Data API` 사용 설정.
5. **저장소 시크릿 둘** — Settings > Secrets and variables > Actions
   - `GA_SA_JSON` — 2번의 JSON 키 **파일 내용 전체**
   - `GA_PROPERTY_ID` — 1번의 숫자

키 JSON 은 대화·노션·저장소 어디에도 붙여 넣지 않는다 (CLAUDE.md §4).

### 언제 도나

| 시각 | 무엇 |
| --- | --- |
| 12:00 KST (`0 3 * * *`) | 낮 집계 — 배포가 통째로 다시 돈다 |
| 24:00 KST (`0 15 * * *`) | 자정 집계 — 원래 있던 일일 배포 |

창은 '오늘' 이 아니라 **최근 24시간**이다. GA4 가 이벤트를 몇 시간 늦게 처리해서, 낮 12시에 '오늘' 을 물으면
오전분이 덜 찬 표가 나온다.

### 확인

```bash
python3 backend/hotsearch_build.py          # 시크릿이 없으면 "빈 표를 씁니다"
...
- **세는 말은 완성어다.** '뮤' 를 치다 뮤츠를 고르면 뮤츠 하나가 오른다. 토막말은 보내지 않는다
  (`frontend-v4/src/lib/track.ts` `trackSearchPick`).
- **통계를 끈 사람은 안 센다.** `track()` 의 동의 게이트를 그대로 탄다.
- **방문이 적으면 GA 가 행을 숨긴다**(임계값). 표가 몇 줄만 나오거나 비는 것은 고장이 아니다.
- **dev 에는 GA 조각이 없다.** 미리보기에서 이 구역은 영영 안 보인다 — 운영에서만 확인한다.
