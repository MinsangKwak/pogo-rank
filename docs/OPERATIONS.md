...
**맞춤 정의 등록 (v2.9.0, 1회)** — 매개변수는 등록하지 않으면 보고서에 안 보입니다. GA 관리 → 데이터 표시 → 맞춤 정의 → 맞춤 측정기준 만들기, 범위 "이벤트"로 다음 6개: `tab` · `page` · `mon` · `boss` · `from` · `status`. 등록 시점 이후 데이터부터 잡히고, 이벤트 보고서에서 보조 측정기준으로 "어떤 포켓몬을 많이 열었나", "도감은 탭 줄과 ☰ 중 어디서 들어오나"를 볼 수 있습니다. 기간은 GA를 붙인 2026-09-03 이후로 잡아야 그래프가 왜곡되지 않습니다.
