...
> v2.18.0부터 GA는 **방문자가 하단 배너에서 동의한 경우에만** 로드됩니다(`components/consent.js`). 동의율만큼 집계가 줄어드는 것이 정상이며, 거부한 사용자는 이벤트가 전혀 오지 않습니다. 배너는 dev·로컬에서도 뜨지만 측정 ID가 없어 선택만 저장됩니다.

측정 ID 는 `G-8MSZM80JHZ` — GA4 속성의 **moncamp.kr 스트림**(스트림 ID 15750161968)입니다. 2026-09-14 v3.27.1 부터 `.github/workflows/deploy.yml` 에 직접 적혀 있습니다(저장소 변수 `GA_ID` 는 더 읽지 않음). 전에는 변수 값이 같은 속성의 `blog` 스트림 측정 ID(`G-KVRX9FBNDC`)라 사이트 통계가 거기 섞여 들어갔습니다 — 그때까지의 데이터는 blog 스트림에 남아 있고, 속성 단위 보고서에서는 합쳐 보입니다. `github.io` · `moncamp.kr` 도메인에서만 로드되어 로컬 개발은 집계되지 않습니다.
