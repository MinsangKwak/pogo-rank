...
| 언제 | 무엇 | 증상 |
| --- | --- | --- |
| ~v3.27.0 | 저장소 변수 `GA_ID` 가 같은 속성의 `blog` 스트림(`G-KVRX9FBNDC`) | 사이트 통계가 blog 스트림에 섞여 쌓임. 속성 단위 보고서에서는 합쳐 보여 안 보였음 |
| v3.27.1~v3.42.0 | 적어 넣은 `G-8MSZM80JHZ`(옛 스트림 `15750161968`)가 그 뒤 사라짐 | `gtag/js` 404 → **집계 0건**. 콘솔에 "데이터 수집이 활성화되어 있지 않습니다" 배너 |

v3.27.0 까지의 데이터는 blog 스트림에 남아 있습니다. v3.27.1~v3.42.0 구간은 **복구할 수 없습니다** — 히트 자체가 나가지 않았습니다.
