...
| **4** ✅ | 인증·권한 서버 | 로그인·회전·권한이 돈다 · 검사 91개 ([`server/AUTH.md`](../server/AUTH.md)) |
| **5** ✅ | 도메인 API | 담아 두기·트레이너 코드·승인이 REST 로 돈다 · 규칙이 막던 한도를 서버가 막는다 · 검사 44개 |
| **6** ✅ | Next.js 앱 | `/mon/25` 가 내용이 든 HTML 로 온다 · 1,207장을 미리 굽는다 · 해시가 사라졌다 |
| **7** ◐ | 배포 이관 | 코드·워크플로·이관 스크립트 끝남 · **누르는 일이 남았다** ([`V5-CUTOVER.md`](V5-CUTOVER.md)) |
| **8** ⏳ | 수집과 지표 | **2026-09-28 전에는 못 켠다** — 방침 10번의 7일 고지 (아래) |

**Firestore 규칙이 지키던 것은 Phase 5 로 전부 넘어왔다.** 신원·세션·삭제는 Phase 4 가, 쓰기 한도와
가입 승인은 Phase 5 가 가져갔다. 개체(`mons`)는 표를 안 만들었으므로 해당 없다 —
v4 화면이 한 번도 안 쓴다(`0004_identity.sql`).

**Phase 8 은 날짜가 막고 있다.** 개인정보처리방침 10번이 "수집 항목이 늘면 시행 7일 전에
패치노트로 알린다" 고 약속했고 그 고지가 2026-09-21 에 나갔다. 그래서 `COLLECT_URL` 은
**2026-09-28 이후에** 켠다 — 코드는 v4.7.1 부터 준비돼 있고, 변수가 비면 화면이 수집을
통째로 끈다. 일찍 켜는 것은 기능을 당기는 것이 아니라 **약속을 어기는 것**이다.
