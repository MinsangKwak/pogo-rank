...
| **8** ✅ | 수집과 지표 | 배포가 켠 채로 굽는다 · 수집→집계→순위를 실제로 돌려 봤다 (아래) |
...
**Phase 8 — 켠 채로 굽는다.** 배포 워크플로가 `COLLECT_URL` 을 넣어 굽고, 안 들어갔으면
**세운다** (비면 화면이 수집을 통째로 끄는데 그건 조용해서 몇 달을 모를 수 있다).

2026-09-22 에 실제로 돌려 봤다 — 서버를 띄우고 브라우저가 보내는 모양 그대로 쐈다.
...
| 표에 IP 칸 | **없음** |
| 롤업(집계) | `written: 3` |
| 한 사람이 친 말 | 순위에 **안 오른다** (`minVisitors: 2`) |
| 자격 갖춘 말이 둘뿐 | 표를 **통째로 비운다** (`minRows: 3`) |
| 열쇠 없이 롤업 | **401** |

고지는 2026-09-21 에 나갔고 패치노트에 적은 시행일은 2026-09-28 이다. 새 화면이 그날
이후에 뜨면 적은 대로이고, 그 전에 뜨면 **패치노트의 날짜를 실제 날짜로 고친다**.
