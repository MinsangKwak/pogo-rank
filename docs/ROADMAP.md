...
| **5** | 도메인 API | 즐겨찾기·트레이너 코드·승인이 REST 로 돈다. **규칙이 막던 쓰기 한도를 서버가 막는다** |
...
**Phase 4 가 옮긴 것과 안 옮긴 것.** Firestore 규칙 검사 11개 중 신원·세션·삭제에 걸린 것은 전부 서버 검사로 왔다.
**쓰기 한도(담아 두기 200/1000)는 판정만 왔고 강제는 Phase 5 다** — 담아 두기 API 가 아직 없어 막을 자리가 없다.
`lib/rbac.ts` 의 `favoriteCap()` 이 그 값을 들고 기다린다. 개체(`mons`)는 표를 안 만들었으므로 해당 없다.
