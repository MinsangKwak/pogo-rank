...
- [1. 설계 원칙](#1-설계-원칙)
- [2. 의사결정 기록 — 어떤 문제를 검토했고 어떻게 바꿨나](#2-의사결정-기록--어떤-고민을-했고-어떻게-바꿨나)
- [3. 데이터 소스](#3-데이터-소스)
...
**27개 항목입니다.** 각 항목은 `문제 → 검토 → 결정 → 결과` 네 항목으로 정리했습니다. 아래 목록에서 필요한 내용을 선택하세요.
...
| [2.28](#228-기록할-자리를-만든다--화면은-그대로-정적-v470) | 기록할 자리를 만든다 — 화면은 그대로 정적 (v4.7.2) |
...
- 월 일정표 — `schedule.js`의 `SCHEDULE_MONTHS`(손)에 `schedule_build.py` 자동분을 합친다 (v4.7.2). 한글 공지·시각을 살리려면 손 줄을 적고, 안 적으면 자동분이 채운다
