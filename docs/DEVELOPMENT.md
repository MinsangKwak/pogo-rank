...
**v3.38.0 — 회원 전용으로 좁혔다. v3.40.0 부터는 관리자 전용이다.** `maxUnrelAllowed()`(`AUTH.admin === true`) 하나를 체크·목록·안내 붙임말 셋이 본다. 비로그인에게는 체크를 아예 안 그리고, 기기 저장값이 켜져 있어도 목록에서 걸러진다. 로그인이 끝나면 D-MAX 를 다시 그린다(`auth.js onAuthChange`) — SDK 가 첫 렌더 뒤에 로드되므로 그러지 않으면 회원이 주소로 바로 들어왔을 때 자리가 안 생긴다(v2.49.0 잠긴 화면과 같은 함정).
