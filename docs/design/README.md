...
`dev`의 `f292a06`에서 분기한 `design` 전용 작업입니다. main·dev·deploy 병합과 배포는 수행하지 않았습니다.
...
- 데이터 계산·인증·권한 정책과 제보 URL은 변경하지 않습니다. 계정·관리자·계산 도구 전체 재설계까지 완료한 것으로 보지 않습니다.
...
- 실제 Firebase 로그인·회원 권한 회귀는 미실시. 로컬 Firebase 설정 없이 디자인을 검토했습니다.
