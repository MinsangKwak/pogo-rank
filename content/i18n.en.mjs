...
export const I18N_PATTERNS = [
  ...
  [/^＋(.+)$/, '＋$1'],
  [/^✓ 계정\((.+)\)에 저장돼요 — 다른 기기에서 로그인해도 같은 화면으로 열려요\.$/, '✓ Saved to your account ($1) — signing in on another device opens the same view.'],
  [/^－(.+)$/, '－$1'],
  [/^(.+) 날짜를 누르면 그날 일정이 보여요\.$/, '$1 Tap a date to see that day\'s events.'],
  [/^(.+) 화면은 승인된 분만 볼 수 있어요\.$/, 'The $1 screen is for approved members only.'],
  [/^(.+) 화면은 관리자가 승인하면 바로 열려요\.$/, 'The $1 screen opens as soon as an admin approves you.'],
  [/^(.+)리그에서 가장 쓸 만해요$/, 'Most useful in the $1 League'],
  [/^(.+)리그 상위 (\d+)$/, '$1 League top $2'],
  [/^(.+)리그 전체 순위$/, '$1 League overall ranking'],
  [/^(.+)리그 · (.+) 타입$/, '$1 League · $2 type'],
  [/^(.+)리그 점수 1위 (.+?)[을를] 중심으로, 서로 약점을 반감해주는 조합을 골랐어요$/,
    'Built around $2, the top-scoring $1 League pick, with teammates that resist each other\'s weaknesses'],
  [/^(.+)리그$/, '$1 League'],
  [/^— 4,096 조합 중 ([\d,]+)위 \(1위의 ([\d.]+)%\)\.$/, '— #$1 of 4,096 combinations ($2% of #1).'],
  [/^1위의 ([\d.]+)%$/, '$1% of #1'],
  [/^1위는 (\d+\/\d+\/\d+)$/, '#1 is $1'],
  [/^(.+) 보스 상대 D-MAX 탱커$/, 'D-MAX tanks vs $1 bosses'],
  [/^(.+) 보스 상대 D-MAX 딜러$/, 'D-MAX attackers vs $1 bosses'],
  [/^(.+) 맥스무브 D-MAX 티어표$/, '$1 Max Move D-MAX tier list'],
  [/^(.+)[가이] 보스로 나오면\? \(맥스 배틀 — 다이맥스·거다이맥스만 참전 가능\)$/, 'If $1 is the boss (Max Battle — only Dynamax · Gigantamax can join)'],
  [/^(.+) 속성 맥스 배틀 보스 기준 · 메가·원시·섀도우는 맥스 배틀에 참전할 수 없어 제외$/,
    'Against a $1-type Max Battle boss · Mega, Primal and Shadow forms cannot join Max Battles and are excluded'],
  [/^(.+) 보스를 상대할 딜러·탱커 순위는$/, 'Attacker and tank rankings vs $1 bosses are under'],
  [/^([SABC]) 티어 · (.+) 맥스$/, '$1 tier · $2 Max'],
  [/^([SABC]) 티어$/, '$1 tier'],
  [/^G·(.+)$/, 'G·$1'],
  [/^(.+) 카운터$/, 'Counters to $1'],
  [/^(.+) 상대 DPS순$/, 'By DPS vs $1'],
  [/^(.+) 자동$/, '$1 (auto)'],
  [/^(.+) \((.+)\) (.+) 기준 \(체력 ([\d,]+) — 티어 고정값\)$/, '$1 ($2) at $3 (HP $4 — fixed per tier)'],
  [/^(.+) · 기절 직전 이탈 → 부활\(5~6초\) → 같은 덱 재진입 · 총 (\d+)사이클 · 부활 (\d+)회 \(부활약·회복약 챙기세요\)$/,
    '$1 · swap out just before fainting → revive (5–6 s) → re-enter with the same team · $2 cycles total · $3 revives (bring Revives and Potions)'],
  [/^(.+) · 기절 직전 이탈 → 부활\(5~6초\) → 같은 덱 재진입 · 총 (\d+)사이클 · 부활 없이 한 번에$/,
    '$1 · swap out just before fainting → revive (5–6 s) → re-enter with the same team · $2 cycles total · in one go, no revives'],
  [/^(.+) \(제한 (\d+)초 중 약 (\d+)초\) · 아래 순서 그대로 내보내면 돼요\.$/, '$1 (about $3 s of the $2 s limit) · send them out in the order below.'],
  [/^(.+) — (이 덱으로는|최정예를 부활시켜 가며 무한정 갈아넣어도) 약 (\d+)초 \((.+)\)\. 친구를 부르거나 풀강·버프로 마진을 채워보세요\.$/,
    '$1 — $2 about $3 s ($4). Invite friends or close the gap with full power-ups and buffs.'],
  [/^(.+) 타입은 (.+?)[이가] 약점 → (.+) 자속 ×([\d.]+)$/, '$1 type is weak to $2 → $3 STAB ×$4'],
  [/^(.+) 자속은 동등\(×1\)$/, '$1 STAB is neutral (×1)'],
  [/^자속\((.+)\)은 ×([\d.]+)로 반감되지만$/, 'STAB ($1) is resisted at ×$2, but'],
  [/^받는 (.+) 공격은 ×([\d.]+) 반감$/, 'incoming $1 attacks are resisted at ×$2'],
  [/^단 (.+) 공격은 ×([\d.]+)로 아프게 받음$/, 'but $1 attacks hit hard at ×$2'],
  [/^(.+)\((\d+)마리 약점\)$/, '$1 ($2 weak to it)'],
  [/^(.+)\(자속 (\d+)종 반감\)$/, '$1 (resists $2 STAB types)'],
  [/^상대: (.+)$/, 'Opponents: $1'],
  [/^공격 기술 추천: (.+) — 이 타입 기술을 가진 픽 위주로\.$/, 'Attack move types: $1 — favor picks carrying these move types.'],
  [/^몸으로 받기 좋은 타입: (.+)$/, 'Good defensive types: $1'],
  [/^역할 분담: (.+)$/, 'Roles: $1'],
  [/^(.+)의 (.+) 약점은 (.+?)[이가] 반감으로 받아줌$/, '$1\'s $2 weakness is covered by $3 resisting it'],
  [/^⚠️ (.+?)[을를] 확실히 이기는 픽이 없어요 — 아래 카운터 목록에서 (.+) 픽으로 한 자리 바꿔보세요\.$/,
    '⚠️ No pick reliably beats $1 — swap one slot for a $2 pick from the counter list below.'],
  [/^⚠️ 둘 이상 같이 아픈 타입: (.+) — 이 타입 상대가 나오면 조심$/, '⚠️ Shared weakness: $1 — watch out for opponents of this type'],
  [/^지금 메타 상위 10마리\((.+) 등\) 상대 평균 상성이 가장 좋은 조합이에요$/, 'The team with the best average matchup against the current top-10 meta ($1 and others)'],
  [/^🃏 추천 덱 (\d+) — (.+)$/, '🃏 Recommended team $1 — $2'],
  [/^💬 (.+)$/, '💬 $1'],
  [/^(.+) 기술$/, '$1 moves'],
  [/^위력↑ (.+)$/, 'Power ↑ $1'],
  [/^위력↓ (.+)$/, 'Power ↓ $1'],
  [/^에너지 (.+)$/, 'Energy $1'],
  [/^신규 (.+)$/, 'New $1'],
  [/^타입: 메가X (.+) ↔ 메가Y (.+)$/, 'Type: Mega X $1 ↔ Mega Y $2'],
  [/^(.+) 종족값 (메가X|메가Y)가 (\d+) 더 높음$/, '$1 base stat: $2 is higher by $3'],
  [/^(왼쪽|오른쪽)의 (.+) 가 더 강한 개체예요!$/, 'On the $1: $2 is the stronger Pokémon!'],
  [/^(왼쪽|오른쪽)의 (.+) 가 지금 더 강해요$/, 'On the $1: $2 is stronger right now'],
  [/^(.+?)[와과] (.+?)[은는] 다른 종이라 나란히 비교할 수 없어요\. (.+?)(?:으로|로) 다시 시작해요\.$/,
    '$1 and $2 are different species and cannot be compared side by side. Starting over with $3.'],
  [/^(.+?)[와과] 비교할 \*\*같은 종\*\* 개체의 \[☐ 비교\] 를 누르세요\.$/, 'Press [☐ Compare] on another **same-species** Pokémon to compare with $1.'],
  [/^(.+) \(Lv ([\d.]+)\) 를 지울까요\?$/, 'Delete $1 (Lv $2)?'],
  [/^(.+) 코드를 삭제할까요\?$/, 'Delete the code for $1?'],
  [/^(.+) 승인을 해제할까요\?$/, 'Revoke approval for $1?'],
  [/^실패 (\d+)개 — (.+)$/, '$1 failed — $2'],
  [/^로그인 실패\(리다이렉트\): (.+)$/, 'Sign-in failed (redirect): $1'],
  [/^로그인 실패: (.+)$/, 'Sign-in failed: $1'],
  [/^초기화 실패: (.+)$/, 'Initialisation failed: $1'],
  [/^계정 삭제 실패: (.+)$/, 'Account deletion failed: $1'],
  [/^허용하지 않은 스크립트 출처: (.+)$/, 'Script origin not allowed: $1'],
  [/^잠긴 화면: (.+)$/, 'Locked screens: $1'],
  [/^내 uid: (.+)$/, 'My uid: $1'],
  [/^⚠ 가입 요청을 저장하지 못했어요 \((.+)\) — 다시 로그인해 보고, 그래도 안 되면 관리자에게 알려주세요$/,
    '⚠ Could not save your sign-up request ($1) — sign in again, and tell the admin if it still fails'],
  [/^(.+) 출처 LeekDuck\(ScrapedDuck\) · (.+) 수집 · 지역과 이벤트에 따라 실제와 다를 수 있어요$/,
    '$1 Source: LeekDuck (ScrapedDuck) · collected $2 · may differ from the game by region and event'],
  [/^(.+) 출처 LeekDuck\(ScrapedDuck\) · 지역과 이벤트에 따라 실제와 다를 수 있어요$/,
    '$1 Source: LeekDuck (ScrapedDuck) · may differ from the game by region and event'],
  [/^(.+) 정보를 아직 받지 못했어요\. 다음 빌드에서 채워져요\.$/, 'No $1 data received yet. It will be filled in the next build.'],
  [/^(.+) 부스트$/, '$1 boost'],
  [/^새 데이터가 있어요 \((.+) 기준\) — 알 부화·레이드 보스가 바뀌었을 수 있어요\.$/,
    'New data is available (as of $1) — egg hatches and raid bosses may have changed.'],
  [/^(.+) 타입 중 "(.+)"$/, '"$2" among $1 types'],
  [/^(.+)\((\d{4})한정\)$/, '$1 ($2 only)'],
  [/^(.+) → (.+)$/, '$1 → $2'],
  [/^([가-힣]+)\(([가-힣·]+)\)$/, '$1 ($2)'],
  [/^([가-힣]+)\/([가-힣]+)$/, '$1/$2'],
...
export const I18N_EN = {
  ...
  "예정된 레이드 일정 보기": "View upcoming raids",
  "현재 기기의 화면 테마: 다크 모드": "Your device currently uses dark mode.",
  "현재 기기의 화면 테마: 밝은 모드": "Your device currently uses light mode.",
  "화면 테마와 포켓몬 이미지의 움직임을 설정할 수 있어요.": "Choose a theme and turn Pokémon animations on or off.",
  "알의 부화 거리와 획득 경로에 따라 나오는 포켓몬을 확인해 보세요.": "Browse hatchable Pokémon by egg distance and how the egg was obtained.",
  "포켓몬의 개체값을 입력하고 리그별 개체값 순위를 확인해 보세요.": "Enter your Pokémon’s IVs to check its IV ranking in each league.",
  "보스의 타입에 맞는 맥스 배틀 포켓몬 3마리를 추천해요.": "Get a team of three Pokémon for Max Battles based on the boss’s type.",
  "기기에 설정된 화면 테마를 따라가요.": "Use your device’s light or dark appearance.",
  "움직이지 않는 공식 일러스트를 표시해요.": "Show static official artwork.",
  "포켓몬 애니메이션": "Pokémon animations",
  "현재 설정은 이 브라우저에 저장돼요.": "Your settings are currently saved in this browser.",
  "계정 이용이 승인되면 저장한 설정을 다른 기기에서도 사용할 수 있어요.": "Once your account is approved, you can use your saved settings on other devices.",
  "헤더의 테마 버튼으로 밝은 모드와 다크 모드를 전환할 수 있어요. 기기 설정을 따르려면 위에서 선택해 주세요.": "Use the theme button in the header to switch between light and dark. Select the device option above to follow your system settings.",
  "아직 즐겨찾기한 포켓몬이 없어요. 도감에서 ★를 눌러 추가해 보세요.": "No favorite Pokémon yet. Tap ★ in the Pokédex to add one.",
  "도감에서 포켓몬을 찾아 즐겨찾기에 추가해 보세요.": "Find a Pokémon in the Pokédex and add it to your favorites.",
  "아직 요약이 준비되지 않은 소식이에요. 위 내용은 공식 발표의 일부이며, 전체 내용은 원문에서 확인할 수 있어요.": "A summary is not available yet. The excerpt above is from the official announcement; follow the source link to read it in full.",
  "업데이트를 찾을 수 없어요. 아직 공개되지 않았거나 게시가 중단되었을 수 있어요.": "This update is unavailable. It may not have been published yet or may have been removed.",
  "전체 일정": "All events",
  "기간 한눈에 보기": "Timeline",
  "일정 보기 방식": "Schedule view",
  "공식 소식부터 배틀에 미치는 영향까지.": "Official updates and what they mean for battles.",
  "확인된 변경 사항을 한곳에서 살펴보세요.": "Explore confirmed changes in one place.",
  ...
  "에 대한 동의를 받아요": "require your consent",
  "아직 승인된 이용자가 없어요.": "No approved users yet.",
  "방문 통계(Google Analytics)는 기능별 이용 현황을 파악하고 화면을 개선하는 데 사용해요. 이메일·이름은 전송하지 않아요.": "Visit analytics (Google Analytics) is used only to see which features get used and improve the screens. Email and name are never sent.",
  "브라우저 저장 항목": "Stored in your browser",
  "로그인 후 관리자 승인을 받으면 이용할 수 있어요.": "Only approved members can use it, so first-timers wait for admin approval.",
  "첫 로그인 때 이용약관·개인정보처리방침에 대한 동의를 받아요.": "On first sign-in we ask you to accept the Terms of Service and Privacy Policy.",
  "계정의 이메일·이름·프로필 사진을 받아요.": "We only receive your email, name and profile photo.",
  "승인 후 이용하기": "Opens once approved",
  "승인 후 계정에 저장한 정보를 다른 기기에서도 사용할 수 있어요.": "After approval, you can access information saved to your account on other devices.",
  "에 대한 동의를 받아요.": "require your consent.",
  "현재 평가 기준에서 가장 높은 등급의 포켓몬이에요.": "The highest-rated Pokémon under the current evaluation criteria.",
  "배틀 조건과 역할에 따라 활용할 수 있는 포켓몬이에요.": "Solid mid-tier picks, depending on the matchup.",
  "현재 평가 기준에서는 다른 후보를 먼저 비교해 보세요.": "Compare other candidates first under the current evaluation criteria.",
  "미출시 포켓몬을 포함한 가상 순위예요. [미구현]을 끄면 출시된 포켓몬 기준으로 볼 수 있어요": "Ranked as if the unreleased entries had launched — turn [not in GO] off for the real ranking",
  "포켓몬 GO의 정보 탐색부터 배틀 준비까지 함께하는 서비스예요.": "Explore Pokémon GO information and prepare for battles.",
  "moncamp는 비공식 팬 프로젝트입니다. Pokémon 및 관련 명칭·이미지의 권리는 The Pokémon Company · Nintendo · Creatures Inc. · GAME FREAK inc.에, Pokémon GO는 Scopely Explore, Inc.에 있으며 이 서비스는 권리자와 무관합니다.": "moncamp is an unofficial fan project. Pokémon and related names and images belong to The Pokémon Company · Nintendo · Creatures Inc. · GAME FREAK inc., and Pokémon GO to Scopely Explore, Inc.; this service is not affiliated with them.",
  "데이터는 PvPoke · PokeMiners · PokeAPI · LeekDuck의 공개 자료를 사용합니다. 코드는 열람용으로 공개하며 포크·재배포를 허용하지 않습니다. 데이터·이미지는 각 출처의 조건을 따릅니다 (저장소 LICENSE · NOTICE).": "Data comes from the public sources PvPoke · PokeMiners · PokeAPI · LeekDuck. The code is published for viewing only — forking and redistribution are not permitted. Data and images follow each source’s terms (see LICENSE and NOTICE in the repository).",
  "아직 등록된 항목이 없어요.": "None yet.",
  "코드 붙여넣기": "Paste to add",
  "공백 없는 12자리 코드로 복사돼요. 게임의 친구 추가 화면에 붙여 넣어 주세요.": "Copied as 12 digits with no spaces — paste it straight into the in-game Add Friend screen.",
  "레벨과 개체값을 입력하면 현재 CP와 레벨 50의 예상 CP를 비교할 수 있어요": "Set the level and IVs of your Pokémon to see its current CP and how much room is left to max",
  "진화형을 선택하면 같은 창에서 해당 포켓몬의 정보를 확인할 수 있어요": "Tap an evolution to switch to it in this window",
  "🧬 PvP 추천 개체값": "🧬 For PvP, aim for these IVs",
  "도감에서 포켓몬 정보를 확인하고 순위표에서 배틀 후보를 비교해 보세요. 즐겨찾기에 담으면 내 포켓몬 화면에서 관련 일정을 확인할 수 있어요.": "Decide what to raise here. Look Pokémon up in the Pokédex, pick recommendations from the rankings, then record your own in the planner.",
  "실험 기능. 추천 덱 3종은 상대 입력 없이 리그 메타 기준으로 뽑아요 — 정석 코어(점수와 약점 보완 기준), 안티 메타(상위 10마리 상대 평균 상성순), 타입 분산(방어 타입 중복 최소화). 맞춤 덱은 PvPoke 리그 순위 × 타입 상성(공격 최대 배율 ÷ 피격 최대 배율)의 근사 추천 — 실드·기술 사이클·CP 최적화는 반영하지 않아요. GO배틀리그 규칙상 같은 종은 파티에 1마리만(섀도우·일반도 같은 종)이라 모든 추천이 종 단위로 중복을 제거해요. 슬롯 3칸을 다 채우면 상대 덱 분석과 구성 가이드가 나와요.": "Experimental. The three suggested teams are drawn from the league meta without any opponent input — Standard Core (score plus greedy weakness cover), Anti-Meta (best average matchup against the top ten), and Type Spread (no overlapping defensive types). Custom team building is an approximation of PvPoke league rank × type matchup (best attacking multiplier ÷ worst incoming multiplier); shields, move cycles and CP optimisation are not modelled. GBL allows only one of each species per team (Shadow counts as the same species), so every suggestion de-duplicates by species. Fill all three slots for an opponent breakdown and a build guide.",
  "즐겨찾기 포켓몬의 일정을 확인하고 게임에서 사용할 검색식을 만들어 보세요.": "Follow your saved Pokémon’s schedule and build search strings to paste into the game.",
  "포켓몬 정보부터 배틀 준비까지, 필요한 기능을 한곳에서 찾아보세요.": "Find Pokémon information and prepare for battles in one place.",
  "이름·타입으로 포켓몬을 찾고 종족값과 상성을 확인해 보세요.": "Look up a Pokémon to see base stats and type matchups.",
  "맥스 배틀에 사용할 포켓몬의 등급과 역할별 순위를 비교해 보세요.": "Compare Max Battle tiers and rankings by role.",
  "레이드에 적합한 공격 포켓몬을 찾고 솔플 가능성을 계산해 보세요.": "Find raid attackers and estimate whether your team can complete a solo raid.",
  "리그별 순위를 비교하고 상대에 맞는 덱을 구성해 보세요.": "Compare league rankings and build a team for your opponents.",
  "공식 발표를 바탕으로 게임 규칙·밸런스 변경과 오류 수정 소식을 전해요.": "Read rule changes, balance updates and bug fixes from official announcements.",
  "레이드와 이벤트 일정을 날짜별로 확인해 보세요.": "Browse raids and events by date.",
  "현재 등장하는 레이드 보스와 약점을 확인해 보세요.": "Raid bosses in rotation right now, and their weaknesses.",
  "알의 거리와 획득 경로별 부화 포켓몬을 확인해 보세요.": "See what hatches from each egg distance.",
  "내 포켓몬의 개체값이 리그별로 몇 위인지 비교해 보세요.": "See where your Pokémon ranks in that league.",
  "상대 포켓몬을 최대 3마리 선택하면 맞춤 덱을 추천해요.": "Select up to three opposing Pokémon to get team recommendations.",
  "보스 타입에 맞춰 맥스 배틀에 사용할 포켓몬 3마리를 추천해요.": "Pick a Max Battle boss and we pick the three to bring.",
  "레이드 보스와 덱을 선택해 혼자 클리어할 수 있을지 계산해 보세요.": "Select a raid boss and your team to estimate whether you can win solo.",
  "원하는 조건을 선택해 포켓몬 GO에서 사용할 검색식을 만들어 보세요.": "Tap conditions to build a string you can paste into the in-game search.",
  "moncamp의 기능 추가와 개선·수정 내역을 확인해 보세요.": "Explore new features, improvements and fixes in moncamp.",
  "이번 시즌의 기술 위력·에너지 변경 내용을 확인해 보세요.": "Moves whose power or energy changed this season.",
  "개인정보의 수집 항목, 이용 목적과 보관·삭제 기준을 안내합니다.": "How information is collected, used, retained and deleted.",
  "서비스 이용 조건과 이용자·운영자의 권리 및 책임을 안내합니다.": "Service terms and the rights and responsibilities of users and the operator.",
  "화면 테마와 이미지 움직임을 설정해 보세요. 계정 저장 여부는 아래에서 확인할 수 있어요.": "Choose your theme and image animation settings. See below for account sync availability.",
  "타입 선택": "Narrow by type",
  "미구현 = 포켓몬 GO에 아직 출시되지 않은 종 (PvPoke 출시 목록 기준, 데이터는 게임마스터 선등록분). ⚡ 메가·원시 배지는 메가진화 또는 원시회귀가 가능한 종을 뜻해요. 포켓몬을 선택한 뒤 진화 탭에서 해당 폼의 능력치를 확인할 수 있어요. 섀도우·리전 폼은 🔍 검색을 통해 이 목록에서 함께 확인할 수 있어요.": "\"Not in GO\" marks species that have not been released in Pokémon GO (per the PvPoke release list; the data is pre-registered in the Game Master). A ⚡ Mega or Primal chip means that species has a Mega Evolution or Primal Reversion — tap the row and the evolution box shows that form’s stats. Shadow and regional forms show up in this list when you find them through 🔍 search.",
  "거다이맥스를 제외하고 다이맥스 포켓몬만 추천해요": "Recommend Dynamax Pokémon only, excluding Gigantamax forms",
  "즐겨찾기 포켓몬": "Marked as favorite",
  "현재 진화 가능한 포켓몬": "Can evolve right now",
  "진화에 도구가 필요한 포켓몬": "Needs an item to evolve",
  "체육관 방어 중인 포켓몬": "Currently in a gym",
  "교환 가능한 포켓몬": "Tradable",
  "교환으로 받은 포켓몬": "Received in a trade",
  "알에서 부화한 포켓몬": "Hatched from an egg",
  "레이드에서 잡은 포켓몬": "Caught in a raid",
  "개체값이 높고 현재 진화 가능한 포켓몬": "High IVs and ready to evolve now",
  "자주 쓰는 검색 조건": "Handy presets",
  "게임에서 검색할 수 없는 교환 상대 닉네임과 리모트 레이드 전용 조건은 제공하지 않아요.": "Left out because the game does not support it: trade partner nickname, remote-raid-only.",
  "사탕 수 등 버전에 따라 지원 여부가 달라지는 조건도 제외했어요.": "Conditions whose support varies by game version, such as candy counts, are excluded.",
  "아직 등록된 게임 업데이트 소식이 없어요. 공식 발표를 확인한 후 게시해요.": "No game updates published yet. Only posts confirmed against an official announcement are shown.",
  "요약 글만 보기": "With summary only",
  "[원문 보기]는 요약이 준비되지 않은 소식이에요. 공식 제목·날짜와 원문 일부를 먼저 제공하며, 확인이 끝나면 요약을 추가해요.": "[Source only] means we have not summarized it yet — you get the official title, date and a verbatim quote. Once a summary is written, that entry becomes a full post.",
  "공식 원문 발췌": "Quoted from the official source",
  "소식을 찾을 수 없어요. 아직 공개되지 않았거나 게시가 중단된 글일 수 있어요.": "That post could not be found. It may not be published yet, or it may have been taken down.",
  "확인할 사항": "Worth checking",
  "moncamp 추천": "What moncamp suggests",
  "등록된 원문 링크가 없어요.": "No source link.",
  "보스를 선택하면 약점과 추천 포켓몬을 확인할 수 있어요. 솔플 가능성은": "Tap a boss for its weaknesses and recommended attackers. To see whether you can solo it, use",
  "에서 확인할 수 있어요.": ".",
  "현재 등장하는 레이드 보스 기준이에요. 예정된 일정은 이벤트 일정에서 확인해 주세요.": "This screen only covers what is in rotation right now — the calendar handles what is coming up.",
  "현재 알에서 부화할 수 있는 포켓몬 목록이에요.": "The egg pools currently in rotation.",
  "종합 점수 순 · 초당 피해량(DPS)과 총 피해량(TDO) 기준": "Ranked by overall score, based on damage per second (DPS) and total damage output (TDO)",
  "이름을 누르면 종족값·상성·배틀 활용 순위를 확인할 수 있어요": "Tap a name for base stats, matchups and every place it is used",
  "필요한 기능을 선택해 바로 시작해 보세요": "Jump straight to the screen for the job",
  "공식 발표로 확인한 게임 변경 소식을 전해요": "Only changes we confirmed against an official announcement",
  "참가 불가": "Cannot enter",
  "참가 가능한 리그가 없어요": "No league fits",
  "CP 상한이 있는 리그에서는 공격 개체값이 낮은 조합": "With a CP cap in play, lower Attack is usually better",
  "이 유리할 수 있어요. 같은 CP 상한 안에서 레벨을 더 높일 수 있기 때문이에요.": " — a lower Attack lets you push the level higher under the same CP cap.",
  "최적 개체값은 포켓몬과 리그에 따라 달라요.": "The best IV spread depends on the Pokémon and league.",
  "포켓몬 선택": "Select a Pokémon",
  "획득 경로": "How obtained",
  "포켓몬을 선택하면 해당 리그의 개체값 순위를 확인할 수 있어요.": "Select a Pokémon to see its IV ranking in the selected league.",
  "이 포켓몬의 종족값 정보가 아직 없어요.": "Base stats are not available for this Pokémon yet.",
  "체력의 소수점 이하를 버리는 게임 규칙을 반영하며, 도감과 동일한 종족값·레벨별 배율을 사용해요.": "HP is floored exactly as the game does it, and base stats and CPM come from the same data this app already uses.",
  "베스트 버디의 레벨 보너스(+1)는 계산에서 제외해요.": "The Best Buddy bonus (+1 level) is not included in this calculation.",
  "★ 레거시 기술 — 대단한 기술머신 또는 특정 이벤트를 통해 습득": "★ Legacy move — only via Elite TM or events",
  "즐겨찾기 포켓몬의 일정을 확인해 보세요": "We keep an eye on your saved Pokémon",
  "포켓몬 상세에서 ★를 눌러 즐겨찾기에 추가해 보세요. 해당 포켓몬의 커뮤니티 데이·스포트라이트 아워·레이드 일정이 등록되면 아래에 표시돼요.": "Tap ★ on a Pokémon page and it lands here. When that Pokémon shows up in a Community Day, Spotlight Hour or raid rotation, you see it in the news panel below first.",
  "즐겨찾기 포켓몬의 일정을 가까운 날짜순으로 보여 드려요.": "Events lined up for the Pokémon you saved, soonest first.",
  "즐겨찾기 포켓몬의 예정된 일정이 없어요. 새 일정이 등록되면 여기에 표시돼요.": "Nothing lined up for your saved Pokémon yet. New events show up here.",
  "이름을 누르면 상세 정보를 보고, ★를 다시 누르면 즐겨찾기에서 삭제할 수 있어요.": "Tap ★ again to remove. Tap a name to open its page.",
  "[도감에서 담을 포켓몬 찾기]를 눌러 즐겨찾기를 추가해 보세요.": "Use [Find a Pokémon in the Pokédex] above to save your first one.",
  "[+]를 눌러 상대 포켓몬을 1~3마리 선택해 보세요. 상대의 타입 상성을 고려한 덱을 추천해요.": "Fill [+] with one to three opponents you keep running into, and we will build a team that answers all of them.",
  "게임 데이터에 등록된 미출시 포켓몬을 포함해요. 빨간 막대로 구분하며, 출시를 가정한 순위가 표시돼요": "Also show entries whose data is in the game files but that cannot be used yet — they get a red bar on the left, and the ranking becomes a what-if with them included",
  "달력에서": "This calendar is",
  "어떤 이벤트가 열리는지 확인해 보세요.": "to see what opens up.",
  "등장하는 보스는": "The bosses actually in rotation are on",
  "이벤트 기간 한눈에 보기": "Event timeline",
  "기기의 밝게·어둡게 설정에 맞춰요.": "Goes dark whenever your phone or computer does.",
  "항상 밝은 테마로 표시해요.": "Always use light mode.",
  "항상 어두운 테마로 표시해요.": "Always use dark mode.",
  "움직이는 도트 이미지를 표시해요. 이미지가 없는 포켓몬은 정지 이미지로 표시하며, 데이터 사용량이 늘어날 수 있어요.": "Pokémon animate as pixel sprites; species without an animated sprite stay still. Uses a little more data.",
  "움직임 없는 공식 일러스트로 표시해요.": "Official artwork stills only.",
  "승인 후에는 계정에 저장한 설정을 다른 기기에서도 사용할 수 있어요.": "Once approved it is saved to your account and matches on every device.",
  "상단의 테마 버튼으로 밝게·어둡게를 전환할 수 있어요. 기기 설정에 맞추려면 여기서 선택해 주세요.": "The theme button in the top bar only flips between light and dark. To follow your device setting, choose it here.",
  "변경한 설정은 다음에 여는 화면부터 적용돼요. 기기의 동작 줄이기 설정이 켜져 있으면 정지 이미지를 표시해요.": "Applies from the next screen you open. If your device has \"reduce motion\" on, images stay still.",
  "실험 기능 · 부활 후 재도전 기준": "Prototype · assumes revives",
  "공격 포켓몬을 추가하면 덱의 예상 클리어 시간을 계산해요.": "Add attackers to estimate your team’s raid completion time.",
  "🙅 계산상 솔플이 어려워요": "🙅 A solo clear looks difficult under these assumptions.",
  "추천 덱을 부활시켜 재투입해도": "even after reviving and re-entering with the recommended team,",
  "아래는 현재 조건에서 예상 클리어 시간이 가장 짧은 구성이에요.": "This team has the shortest estimated completion time under the selected conditions.",
  "관리자가 실험 기능 이용 권한을 부여하면 사용할 수 있어요.": "You can use this feature once an administrator grants experimental access.",
  "CP 상한이 있는 리그에서는 공격 개체값이 낮을 때 레벨을 더 높일 수 있는 경우가 있어요. 따라서 개체값 100%가 항상 1위는 아니에요.": "In leagues with a CP cap, lower Attack IVs may allow a higher level. Perfect IVs do not always rank first.",
  "굵은 숫자는 표시된 레벨에서 개체값이 100%(15/15/15)일 때의 CP예요. 특히 야생 포켓몬은 레벨이 다를 수 있으므로 CP만으로 개체값을 확정할 수 없어요.": "Bold numbers show CP for perfect IVs (15/15/15) at the listed level. Wild Pokémon may have different levels, so CP alone cannot confirm their IVs.",
  "이 종의 종족값 정보가 아직 없어요.": "Base stats are not available for this Pokémon yet.",
  // ── 서비스 · 이동 ──────────────────────────────────────────────────────────
  '서비스 홈': 'Home',
  '내 포켓몬': 'My Pokémon',
  '포켓몬 도감': 'Pokédex',
  ...
  '켜기': 'On',
  '끄기': 'Off',
  '포켓몬이 도트 그림으로 움직여요. 움직이는 그림이 없는 종은 정지 그림 그대로예요. 데이터를 조금 더 써요.': 'Pokémon animate as pixel sprites; species without an animated sprite stay still. Uses a little more data.',
  '공식 일러스트 정지 그림만 써요.': 'Official artwork stills only.',
  ...
  // ── 2026-09-14 v3.26.0 엄격 감사 — 화면·팝업을 EN 으로 두루 열어 남은 한국어 ──────────
  // 로그인 유도 팝업 · 잠긴 화면 카드 · 계정 카드 (auth.js · planner/shell.js · app.js)
  '로그인하면 열려요': 'Opens after sign-in',
  '로그인하면 열려요. 이 화면은 승인된 분만 볼 수 있어요.': 'Opens after sign-in. This screen is for approved members only.',
  '이 화면은 승인된 분만 볼 수 있어요.': 'This screen is for approved members only.',
  '이 화면은 관리자가 승인하면 바로 열려요.': 'This screen opens as soon as an admin approves you.',
  '승인을 기다리는 중이에요': 'Waiting for approval',
  '관리자가 승인하면 바로 열려요.': 'Opens as soon as an admin approves you.',
  '승인된 분만 쓸 수 있어서, 처음이라면 관리자 승인을 기다리게 돼요.': 'Only approved members can use it, so first-timers wait for admin approval.',
  '🔐 Google로 로그인': '🔐 Sign in with Google',
  'Google 계정으로 로그인': 'Sign in with your Google account',
  '이메일·이름·프로필 사진만 받아요.': 'We only receive your email, name and profile photo.',
  '관리자 승인 기다리기': 'Wait for admin approval',
  '바로 승인되지 않을 수 있어요. 관리자에게 알려 주세요.': 'Approval may not be immediate. Let the admin know.',
  '승인되면 열려요': 'Opens once approved',
  '확인': 'OK',
  '나중에': 'Later',
  '첫 로그인 때': 'On first sign-in, the',
  '동의를 받아요.': 'require your consent.',
  '동의를 받아요': 'require your consent',
  '첫 로그인 때 이용약관·개인정보처리방침 동의를 받아요.': 'On first sign-in we ask you to accept the Terms of Service and Privacy Policy.',
  '로그인하면 내 포켓몬과 화면 설정이 계정에 저장돼요. 승인된 분만 쓸 수 있고, 첫 로그인 때':
    'Sign in and your Pokémon and display settings are saved to your account. Access is by approval, and on first sign-in the',
  '승인제라 로그인해도 바로 열리지 않을 수 있어요. 받는 정보는 이메일 · 이름 · 프로필 사진뿐이고, 첫 로그인 때':
    'Access is approved by hand, so signing in may not open everything right away. We only receive your email, name and profile photo, and on first sign-in the',
  '로그인하면 내 개체를 계정에 저장하고 같은 종끼리 비교할 수 있어요. 승인된 분만 쓸 수 있어요.':
    'Sign in to save your Pokémon to your account and compare ones of the same species. Approved members only.',
  'SDK 를 받지 못했어요 — 네트워크를 확인해 주세요': 'Could not load the sign-in SDK — check your network',
  '테스트 목(dev-mock.js) 로드 실패 — dist/ 를 다시 빌드하세요': 'Failed to load the test mock (dev-mock.js) — rebuild dist/',
  '로그인이 끝까지 가지 못했어요 — 이 브라우저가 사이트 간 저장소를 막고 있어요. [Google로 로그인] 을 한 번 더 눌러 팝업으로 시도해 주세요.':
    'Sign-in did not complete — this browser blocks cross-site storage. Press [Sign in with Google] once more to try a popup.',
  '로그인 준비 중… 잠시 후 다시 눌러주세요': 'Preparing sign-in… try again in a moment',
  '관리자 계정은 여기서 지울 수 없어요 — 먼저 ADMIN_UID 를 다른 계정으로 옮기세요': 'The admin account cannot be deleted here — move ADMIN_UID to another account first',
  '계정을 지우는 중…': 'Deleting account…',
  '계정과 저장 데이터를 지웠어요. 다시 로그인하면 새 계정(승인 대기)으로 시작해요': 'Your account and saved data were deleted. Signing in again starts a new account (awaiting approval)',
  '— 문의처로 알려 주시면 지워 드릴게요': '— contact us and we will delete it for you',
  '승인 정보와 가입 요청(이메일·이름·사진·약관 동의 기록)': 'Approval record and sign-up request (email · name · photo · terms consent)',
  'Google 로그인 연결(Firebase 인증 계정)': 'Google sign-in link (Firebase auth account)',
  '🗑 지우기 (되돌릴 수 없음)': '🗑 Delete (cannot be undone)',
  '계정을 삭제할까요?': 'Delete your account?',
  '아래가 바로 지워지고 되돌릴 수 없어요. 브라우저에 남은 설정값(마지막 탭 등)은 개인정보가 아니라 그대로 둬요.':
    'The items below are deleted immediately and cannot be recovered. Settings left in the browser (last tab and so on) are not personal data and stay as they are.',
  '취소': 'Cancel',
  '(이름 없음)': '(no name)',
  ...
  '승인된 친구': 'Approved friends',
  '아직 승인된 친구가 없어요.': 'No approved friends yet.',
  '내 uid 복사': 'Copy my uid',
  '🎒 내 포켓몬 #마리 · 화면 설정': '🎒 My Pokémon (#) · display settings',
  '🎒 내 포켓몬': '🎒 My Pokémon',

  // 트레이너 코드 관리 (trainers.js)
  '복사 실패': 'Copy failed',
  '아직 등록된 코드가 없어요. 아래 “코드 관리”에서 추가하세요.': 'No codes registered yet. Add them under “Manage codes” below.',
  '아직 등록된 코드가 없어요.': 'No codes registered yet.',
  '🛠 트레이너 코드 관리': '🛠 Manage trainer codes',
  '아직 없어요.': 'None yet.',
  '한 줄에 하나씩\n이름 # # # #\n이름# # # # #': 'One per line\nName # # # #\nName# # # # #',
  '이름과 #자리 코드를 한 줄에 하나씩. 같은 이름이 있으면 덮어써요.': 'Name and #-digit code, one per line. An existing name is overwritten.',
  '일괄 저장': 'Save all',
  '입력된 줄이 없어요.': 'No lines entered.',
  '저장 중…': 'Saving…',
  '응답 없음(네트워크 확인)': 'No response (check your network)',
  '→ Firebase 콘솔 > Firestore > 규칙에 trainers 블록을 게시했는지 확인하세요.': '→ Check that the trainers block is published under Firebase console > Firestore > Rules.',
  '붙여넣어 추가': 'Paste to add',
  '삭제': 'Delete',
  '#개 저장': '# saved',
  ...
  '이용 패턴을 기록해요': 'Usage patterns are recorded',
  '이 빌드(미리보기·로컬)는 통계가 꺼져 있어 선택만 저장돼요': 'Analytics is off in this build (preview · local), so only your choice is saved',
  '통계 스크립트를 불러오지 않아요': 'The analytics script is not loaded',
  '브라우저에 저장된 것': 'Stored in your browser',
  '오프라인용 파일 캐시(화면·데이터·포켓몬 그림) — 서비스워커': 'Offline file cache (screens · data · Pokémon art) — service worker',
  '설정값(마지막 탭·모드, 패치노트 읽음, 동의 여부 등) — localStorage, pogo_ 접두사': 'Settings (last tab · mode, patch notes read, consent and so on) — localStorage, pogo_ prefix',
  '위치정보는 수집하지 않아요': 'No location data is collected',
  '🧹 캐시 비우고 새로고침': '🧹 Clear cache and reload',
  '설정값까지 지우려면 브라우저의 "사이트 데이터 삭제"를 쓰세요. 계정에 저장한 즐겨찾기·내 포켓몬은 여기서 지워지지 않아요 (계정 카드 → 계정 삭제).':
    'To clear settings too, use your browser\'s "Delete site data". Favorites and Pokémon saved to your account are not removed here (account card → delete account).',

  // 상세 팝업 (detail.js · modal.js · row.js)
  '상세 정보': 'Details',
  '상세 보기': 'View details',
  '없음': 'None',
  '이중': 'Double',
  '약점 (더 큰 데미지)': 'Weaknesses (takes more damage)',
  '내성 (덜 받는 데미지)': 'Resistances (takes less damage)',
  '이중 = 두 타입 모두에 걸려 ×#(약점) / ×#(내성·무효)': 'Double = hit by both types ×# (weak) / ×# (resist · immune)',
  'CP #% 기준': 'CP at #% IVs',
  '최저 #': 'Min #',
  '맥스 배틀 (거다이맥스·다이맥스) — Lv# 고정, 날씨부스트 없음': 'Max Battle (Gigantamax · Dynamax) — fixed Lv#, no weather boost',
  '굵은 숫자가 개체값 #%(#/#/#) CP예요. 잡은 개체가 이 값이면 #%. 야생은 레벨 하한이 없어 최저 CP를 적지 않아요.':
    'Bold numbers are the CP at #% IVs (#/#/#). A catch with this CP is #%. Wild spawns have no level floor, so no minimum CP is listed.',
  '굵은 숫자가 개체값 #%(#/#/#) CP예요. 잡은 개체가 이 값이면 #%. 맥스 배틀은 날씨부스트가 없어 항상 Lv#이라 레이드 평시와 같은 CP가 나와요. 야생은 레벨 하한이 없어 최저 CP를 적지 않아요.':
    'Bold numbers are the CP at #% IVs (#/#/#). A catch with this CP is #%. Max Battles have no weather boost, so they are always Lv# and give the same CP as a normal raid. Wild spawns have no level floor, so no minimum CP is listed.',
  '각 순위표 상위 #위 기준 · #위 안은 강조 표시': 'Based on the top # of each ranking · top # highlighted',
  '진화형을 누르면 그 포켓몬의 정보를 볼 수 있어요 · ⚡ 메가 진화 가능 — 누르면 메가 진화 스탯을 볼 수 있어요':
    'Tap an evolution to view that Pokémon · ⚡ Can Mega Evolve — tap to see Mega stats',
  '메가X': 'Mega X',
  '메가Y': 'Mega Y',
  '⚔️ #-#-# 기술 변경 예정 (D-#)': '⚔️ Move changes coming #-#-# (D-#)',
  '⚔️ #-#-# 기술 변경 적용됨': '⚔️ Move changes applied #-#-#',

  // D-MAX (views/max.js)
  'D-MAX 탱커 (중립 · 순수 내구)': 'D-MAX tanks (neutral · raw bulk)',
  'D-MAX 딜러 (중립 · 맥스 피해 × √내구)': 'D-MAX attackers (neutral · max damage × √bulk)',
  '보스 속성': 'Boss type',
  '상위 #': 'Top #',
  '맥스 피해 · 내구 #': 'Max damage · bulk #',
  '맥스 피해 #': 'Max damage #',
  '받는 배율 ×#': 'damage taken ×#',
  'EHP · 체력 # × 방어 #': 'EHP · HP # × Defense #',
  ...
  '[탱커]': '[Tanks]',
  '에서': '.',
  '탱커 순위: EHP = 체력 × 방어 ÷ # ÷ (보스 타입 기술을 받는 배율). 레벨 # 실전 능력치 기준이고, 보스는 자기 타입 자속 기술로 때린다고 가정해요(복합 타입 보스는 상세 팝업의 타입 상성을 함께 보세요). 출시된 다이맥스·거다이맥스만 포함. 포켓몬을 누르면 상세 정보가 열려요.':
    'Tank ranking: EHP = HP × Defense ÷ # ÷ (damage multiplier from the boss type\'s moves). Uses level # battle stats and assumes the boss attacks with STAB moves of its own type (for dual-type bosses also check the type matchups in the detail popup). Only released Dynamax · Gigantamax are included. Tap a Pokémon for details.',
  '위는 티어표(그 타입 맥스무브를 쓰는 개체), 아래는 딜러(그 타입 보스를 상대할 개체) — 같은 타입을 골라도 보는 각도가 달라 명단이 달라요. 티어표는 공격 종족값 × 맥스무브 위력(거다이 # · 다이 #) × 자속 #, 내구 미반영이고 행을 누르면 근거가 펼쳐져요. 딜러 순위는 맥스어택 #레벨(위력 #) 또는 거다이맥스 #레벨(위력 #) #회 피해 × √내구 기준이며, 보스 속성을 고르면 그 속성 보스를 때릴 때의 상성이 반영돼요(전체는 중립). 출시된 다이맥스·거다이맥스만 포함. 포켓몬을 누르면 상세 정보가 열려요.':
    'Top: the tier list (Pokémon using that type\'s Max Move). Bottom: attackers (Pokémon to bring against a boss of that type) — the same type gives different lists because the angle differs. The tier list is Attack base stat × Max Move power (Gigantamax # · Dynamax #) × STAB #, ignoring bulk; tap a row for the reasoning. Attacker ranking is one hit of Max Attack level # (power #) or Gigantamax level # (power #), × # then × √bulk; choosing a boss type applies the matchup against that type (All = neutral). Only released Dynamax · Gigantamax are included. Tap a Pokémon for details.',

  // 솔플 계산기 · 추천 덱 (views/ifsolo.js)
  '데이터가 없어요.': 'No data.',
  '시간 여유 #%': 'Time to spare #%',
  '약 #% 부족': 'About #% short',
  '💪 솔플 가능 — 정예 #마리, 약 #초': '💪 Solo possible — # elite picks, about # s',
  '🙅 이건 사람 손으로는 무리예요': '🙅 No one can pull this off by hand',
  '이 덱으로는': 'with this team,',
  '최정예를 부활시켜 가며 무한정 갈아넣어도': 'even cycling the very best picks with unlimited revives,',
  '아래는 그래도 가장 빨리 깎는 구성이에요.': 'Below is still the fastest team for chipping it down.',
  '제한 시간 내 이 덱의 총 딜 약 # / 보스 체력 # → 딜 여유 # (+#%)': 'Total damage within the time limit about # / boss HP # → # to spare (+#%)',
  '제한 시간 내 이 덱의 최대 딜 약 # / 보스 체력 # → # (#%) 모자라서 못 잡음': 'Max damage within the time limit about # / boss HP # → # (#%) short, cannot win',
  '필요 개체 #마리 (이 순서로)': '# Pokémon needed (in this order)',
  '검색 결과가 없어요.': 'No results.',
  '레이드 CP #': 'Raid CP #',
  '풀강 최대 CP #': 'Max CP fully powered #',
  '탭하면 난이도 수동 변경': 'Tap to change the tier by hand',
  '선택 해제': 'Clear selection',
  '내 어태커 검색해서 추가 (예: 자시안, 메가Y 뮤츠)': 'Search and add your attackers (e.g. Zacian, Mega Mewtwo Y)',
  '이 보스 상대 상위 목록에 없어 평가할 수 없는 포켓몬이에요.': 'Not in the top list against this boss, so it cannot be rated.',
  '즐겨찾기 중 이 보스 상대 상위 목록에 든 포켓몬이 없어요.': 'None of your favorites are in the top list against this boss.',
  '덱이 이미 #마리예요.': 'The team already has # Pokémon.',
  '로그인하면 ★ 즐겨찾기로 덱을 바로 채울 수 있어요': 'Sign in to fill the team straight from ★ Favorites',
  '★ 즐겨찾기에서 채우기 (#)': '★ Fill from favorites (#)',
  '내 덱': 'My team',
  '#/# · 넣는 순서대로 출전': '#/# · sent out in the order added',
  '누르면 덱에서 제거': 'Tap to remove from the team',
  '어태커를 추가하면 이 조합으로 잡을 수 있는지 알려 줘요.': 'Add attackers and we tell you whether this team can win.',
  '받는 공격은 ×1': 'incoming attacks are ×1',
  '🧠 상대 덱 분석 & 구성 가이드': '🧠 Opponent analysis & team guide',
  '확실한 우위 없음': 'no clear edge',
  '✅ 상대 #마리 모두 상성 우위 픽이 있는 구성이에요.': '✅ This team has a favorable pick against all # opponents.',
  '둘 이상 같이 아픈 타입이 없어 한 상성에 쓸리지 않음': 'No weakness shared by two or more, so one matchup cannot sweep the team',
  '방어 타입이 겹치지 않아 상대가 한 타입 기술로 셋을 다 뚫지 못해요': 'Defensive types do not overlap, so no single move type breaks all three',
  '누르면 제거': 'Tap to remove',
  '맞춤 추천 덱': 'Custom recommended team',
  '상대 #마리 기준': 'vs # opponents',
  '상성 계수': 'Matchup factor',
  '상성 계수 ×#': 'Matchup factor ×#',
  '정석 코어': 'Standard core',
  '점수 상위 + 약점 상호 보완': 'Top score + complementary weaknesses',
  '리그 상위 10마리 저격': 'Targets the league top 10',
  '타입 분산': 'Type spread',
  '방어 타입 안 겹침': 'No overlapping defensive types',
  ...
  '상승': 'up',
  '하락': 'down',
  '#-#-# 갱신에서 #계단 상승': 'In the #-#-# update: up # places',
  '#-#-# 갱신에서 #계단 하락': 'In the #-#-# update: down # places',
  '지금은 예정된 기술 변경이 없어요.': 'No move changes are scheduled right now.',
  '적용 전이라 순위표에는 아직 반영돼 있지 않아요. 적용 다음 날 자동 갱신되면 순위가 움직인 포켓몬에 ▲▼ 표시가 붙어요. 레이드 티어표를 움직이는 것은 사이코부스트(체육관·레이드 # → #)와 새로 배우는 기술 쪽이에요.':
    'Not applied yet, so the rankings do not reflect it. After the automatic update the day after it lands, Pokémon whose rank moved get a ▲▼ mark. What shifts the raid tier list is Psycho Boost (gym · raid # → #) and the newly learned moves.',
  '⚔️ 기술 변경 예고 (#/# · D-#)': '⚔️ Upcoming move changes (#/# · D-#)',
  '⚔️ #/# 기술 변경': '⚔️ Move changes on #/#',
  '위력 # → #': 'Power # → #',
  '#-#-# 적용 · D-#': 'Applies #-#-# · D-#',
  '#-#-# 적용됨': 'Applied #-#-#',
  '새로 배우는 기술 · #건': 'Newly learned moves · #',
  ...
  '순위 없음': 'Unranked',
  '못 들어가요': 'Cannot enter',
  'Lv# CP 가 이미 상한을 넘어요': 'Lv# CP already exceeds the cap',
  '들어갈 리그가 없어요': 'No league fits',
  '상한이 있는 리그는 Lv# CP 가 이미 넘어요.': 'Every capped league is already exceeded at Lv# CP.',
  '마스터리그는 CP 상한이 없어요 — 개체값이 높을수록 좋아서 순위를 매기지 않아요.': 'Master League has no CP cap — higher IVs are simply better, so there is no ranking.',
  '이 리그에는 들어갈 수 없어요 — Lv# CP 가 이미 상한을 넘어요.': 'Cannot enter this league — Lv# CP already exceeds the cap.',
  '이 종의 종족값이 아직 없어요.': 'No base stats for this species yet.',
  '개체값이 높을수록 좋아요 · Lv# CP #': 'Higher IVs are better · Lv# CP #',
  'CP # · 개체값 조합 #개 중': 'CP # · among # IV combinations',
  '순위 #위': 'Rank #',
  '🧬 PvP 라면 이 개체값': '🧬 For PvP, aim for these IVs',
  'PvP 는 CP 상한이 있어 공격이 낮을수록 레벨을 더 올릴 수 있어요. 그래서 #% 개체가 #위가 아니에요.': 'PvP has CP caps, so lower Attack lets you reach a higher level. That is why a #% Pokémon is not rank #.',
  '🧬 PvP 개체값 순위 화면에서 내 개체가 몇 위인지 볼 수 있어요.': '🧬 See where your own Pokémon ranks on the PvP IV Ranking screen.',
  '다른 종 고르기': 'Choose another species',
  '공격 # · 방어 # · 체력 #': 'Attack # · Defense # · HP #',
  '공격 # · 방어 # · 체력 # · 맥스 배틀 다이맥스 가능': 'Attack # · Defense # · HP # · Max Battle: Dynamax capable',
  ...
  '이 폼은 능력치 데이터가 없어 저장할 수 없어요.': 'This form has no stat data and cannot be saved.',
  '게임에 보이는 CP 를 넣으세요': 'Enter the CP shown in the game',
  '레벨 추정': 'Estimate level',
  'Lv # 로 맞췄어요': 'Set to Lv #',
  '정확히 맞는 레벨이 없어요 — 가장 가까운 Lv # (CP 차이 #). 개체값을 다시 봐 주세요': 'No level matches exactly — closest is Lv # (CP off by #). Check the IVs again',
  '모름 / 아직': 'Unknown / not yet',
  '메모 (예: 레이드용 #호기, 교환 받은 것)': 'Note (e.g. raid backup #, received in a trade)',
  '섀도우 개체': 'Shadow Pokémon',
  '# ~ #, # 단위': '# to #, in steps of #',
  'CP 로 맞추기': 'Set from CP',
  '상태': 'Status',
  '메모': 'Note',
  '저장': 'Save',
  '내 포켓몬에 추가': 'Add to My Pokémon',
  '개체는 #마리까지 저장할 수 있어요.': 'You can save up to # Pokémon.',
  'CP 는 저장하지 않고 종족값 × 레벨 × 개체값으로 계산해요. * 는 레거시 기술. 섀도우는 CP 가 같고 배틀에서만 공격 ×# · 방어 ×#.':
    'CP is not stored; it is calculated from base stats × level × IVs. * marks legacy moves. Shadow Pokémon have the same CP and differ only in battle: Attack ×# · Defense ×#.',
  '· 개체값 #% · 만렙 CP # (#%)': '· IVs #% · max CP # (#%)',
  '섀도우와 일반은 이 표만으로 못 골라요': 'This table alone cannot decide between Shadow and normal',
  '섀도우는 CP 에 안 잡히는 배틀 보정(공격 ↑ · 방어 ↓)이 따로 있어요. 레이드 딜러라면 섀도우가, 오래 버텨야 하면 일반이 유리한 편이에요.':
    'Shadow Pokémon have a battle modifier (Attack ↑ · Defense ↓) that CP does not show. Shadow tends to win for raid attackers, normal for anything that has to last.',
  '왼쪽': 'Left',
  '오른쪽': 'Right',
  '다 키웠을 때(Lv #) CP 가 # 더 높아요. 육성 대상을 하나만 고른다면 이쪽이에요.': 'Fully powered (Lv #) its CP is # higher. If you raise only one, this is it.',
  '다 키우면 같은 CP 가 되지만, 지금 CP 가 더 높아 바로 데려가 쓰기 좋아요. 강화 비용도 그만큼 덜 들어요.':
    'Both end at the same CP fully powered, but this one is higher right now, so it is ready sooner and costs less to power up.',
  '두 개체가 같아요': 'The two are identical',
  '개체값도 도달 CP 도 같아요. 기술이나 메모처럼 표 밖의 기준으로 고르세요.': 'Same IVs and same reachable CP. Decide on something outside this table, like moves or your notes.',
  '⚖️ 같은 종 개체 비교': '⚖️ Compare two of the same species',
  '같은 종을 나란히 놓고 어느 개체가 더 좋은지 봐요.': 'Put two of the same species side by side to see which is better.',
  '개체값 #/#/# (#%)': 'IVs #/#/# (#%)',
  '개체값 #%': 'IVs #%',
  '개체값 공/방/체': 'IVs Atk/Def/HP',
  '만렙 CP (Lv#)': 'Max CP (Lv#)',
  '불가': 'N/A',
  '스피드 기술': 'Fast move',
  '차지 기술': 'Charged move',
  '굵은 값이 더 큰 쪽이에요. 리그 도달 = CP 상한을 넘지 않는 가장 높은 레벨의 CP. 이 표는 같은 종 안에서의 숫자 비교일 뿐 순위표 평가가 아니며, PvP 에서는 개체값이 낮아도 상한에 딱 맞는 개체가 유리할 수 있어요.':
    'Bold marks the higher value. League reach = CP at the highest level that stays under the cap. This only compares numbers within one species, not ranking quality; in PvP a lower-IV Pokémon that lands right under the cap can be the better one.',
  '만렙 기준 백개체 CP # 의 #% (# 차이)': 'Hundo max CP #: this one reaches #% (# apart)',
  '✨ 거다이맥스 가능': '✨ Gigantamax capable',
  '✨ 다이맥스 가능': '✨ Dynamax capable',
  '폼 데이터가 없어 CP 를 계산할 수 없어요': 'No form data, so CP cannot be calculated',
  '스피드 모름': 'Fast move unknown',
  '차지 모름': 'Charged move unknown',
  '다른 종이라 지금 고른 개체와는 비교할 수 없어요': 'Different species — cannot compare with the one selected',
  'CP 계산만 해 볼 수 있어요. 이 빌드는 로그인 기능이 꺼져 있어 저장이 안 돼요.': 'You can only calculate CP here. Sign-in is disabled in this build, so nothing is saved.',
  '⏳ 승인 대기 중 — 계산은 지금도 돼요. 승인되면 개체를 계정에 저장할 수 있어요.': '⏳ Awaiting approval — calculation already works. Once approved you can save Pokémon to your account.',
  '계산은 로그인 없이도 돼요. ☰ 메뉴 맨 위 "👤 마이페이지" 에서 로그인하면 개체를 계정에 저장해 어느 기기에서든 같은 목록을 봐요.':
    'Calculation works without signing in. Sign in from “👤 My Page” at the top of the ☰ menu to save Pokémon to your account and see the same list on every device.',
  '이 상태의 개체가 없어요.': 'No Pokémon with this status.',
  '전체 #': 'All #',
  '육성 중 #': 'Raising #',
  '완료 #': 'Done #',
  '교환 후보 #': 'Trade candidate #',
  '계산·조회는 할 수 있어요. 이 빌드는 로그인 기능이 꺼져 있어 저장만 안 돼요.': 'Calculation and lookup work. Sign-in is disabled in this build, so only saving is off.',
  '⏳ 승인 대기 중 — 승인되면 개체를 계정에 저장하고 기기 간에 동기화해요.': '⏳ Awaiting approval — once approved, Pokémon are saved to your account and synced across devices.',
  '☰ 메뉴 맨 위 "👤 마이페이지" 에서 로그인하면 개체를 계정에 저장하고 어느 기기에서든 같은 목록을 봐요. 로그인 없이도 CP 계산은 해 볼 수 있어요.':
    'Sign in from “👤 My Page” at the top of the ☰ menu to save Pokémon to your account and see the same list on every device. CP calculation works without signing in.',
  '아직 저장한 개체가 없어요. 위 버튼으로 첫 개체를 등록해 보세요.': 'No Pokémon saved yet. Use the button above to add your first one.',
  'Lv # · 개체값 #/#/# (#%) · CP #': 'Lv # · IVs #/#/# (#%) · CP #',
  '내 포켓몬 #마리': 'My Pokémon: #',

  // 화면 설명 (router.js) · 패치노트 머리
  ...
  '개체값 #% (네 별)': '#% IVs (four stars)',
  '지금 진화할 수 있는 것': 'Can evolve right now',
  '세대': 'Gen',
  '새 버전이 나왔어요.': 'A new version is available.',
  '새로고침': 'Reload',

  // 레이드 보스 · 알 부화 (gameday.js) · PvE (pve.js · tier.js) · PvP (pvp.js) · 홈 (home.js)
  '어드벤처 싱크 보상': 'Adventure Sync rewards',
  '루트 선물': 'Route gifts',
  '친구 선물': 'Friend gifts',
  '매 초당 가하는 평균 피해량': 'Average damage dealt per second',
  '쓰러질 때까지 가하는 누적 피해량': 'Total damage dealt before fainting',
  'DPS와 TDO를 함께 반영한 종합 효율 지표': 'Overall efficiency combining DPS and TDO',
  '점수': 'Score',
  '종합(%) — 최강 어태커 대비 백분위': 'Overall (%) — percentile vs the best attacker',
  '평가': 'Rating',
  'DPS·TDO·종합 순서의 등급, S > A > B > C (예: SAS)': 'Grades in DPS · TDO · overall order, S > A > B > C (e.g. SAS)',
  '레거시 기술 — 대단한 기술머신 또는 이벤트로만 습득': 'Legacy move — only via Elite TM or events',
  '출처: hawaii 「속성별 레이드 성능표」(구글 시트, 매일 #시 자동 수집). 같은 포켓몬이 두 번 나오면 기술 조합이 다른 것. 포켓몬을 누르면 상세 정보가 열려요.':
    'Source: hawaii “Raid performance by type” (Google Sheet, collected daily at #:00). The same Pokémon listed twice means a different moveset. Tap a Pokémon for details.',
  '자체 계산: 레벨 #, 개체값 #/#/#, 보스 방어 #·초당 피해 # 가정. 정렬은 DPS³×TDO 종합 점수 기준, 표시는 DPS. 포켓몬을 누르면 상세 정보가 열려요.':
    'Own calculation: level #, IVs #/#/#, assuming boss Defense # · # damage taken per second. Sorted by the DPS³×TDO overall score, showing DPS. Tap a Pokémon for details.',
  '속성별 레이드 성능표 기준': 'Based on the raid performance table by type',
  '자체 계산 · 상위 #': 'Own calculation · top #',
  '#종 · 전설·환상·메가·섀도우 제외': '# species · excluding Legendary · Mythical · Mega · Shadow',
  'CP # · 상위 # 기준': 'CP # · top # shown',
  '전체 #위': 'Overall #',
  '🧬 개체값 순위': '🧬 IV ranking',
  'DPS # · 버팀 #': 'DPS # · TDO #',
  ...
  '개체값 #%': 'IVs #%',
  '초기화': 'Reset',
  // 2026-09-16 v3.51.0 📢 게임 업데이트 — 화면의 뼈대만 옮긴다.
  // 기사 본문은 공식 공지를 한국어로 요약한 글이라 사전을 태우지 않고, i18nKoOnlyNote('ko') 로 그 사실을 먼저 밝힌다
  '게임 업데이트': 'Game updates',
  '📢 게임 업데이트': '📢 Game updates',
  '게임의 규칙·밸런스·오류가 바뀐 소식이에요. 공식 발표를 확인한 것만 적어요.':
    'News about rule, balance and bug changes in the game. Only what we confirmed against an official announcement.',
  '게임에서 무엇이 바뀌었는지 확인한 것만 적어요': 'Only changes we confirmed against an official announcement',
  '전체 보기 ›': 'See all ›',
  '주요 변경': 'Highlighted changes',
  '전체 소식': 'All updates',
  '제목·내용으로 찾기': 'Search by title or content',
  '게임 업데이트 검색': 'Search game updates',
  '전체 분류': 'All categories',
  '체육관': 'Gyms',
  '레이드·맥스': 'Raids & Max',
  '포획·육성': 'Catching & training',
  '보상·편의': 'Rewards & quality of life',
  '오류 수정': 'Bug fixes',
  '최근 #일': 'Last # days',
  '#건': '#',
  '공식 확인': 'Official',
  ...
  '적용 확인': 'Live',
  '철회': 'Withdrawn',
  '시점 미확인': 'Timing unknown',
  '발표': 'Announced',
  '적용': 'Effective',
  '마지막 확인': 'Last checked',
  '날짜 미확인': 'Date unknown',
  '핵심 요약': 'Key points',
  '변경 전 · 후': 'Before and after',
  '이전 값 미확인': 'Previous value unknown',
  '플레이에 미치는 영향': 'What it changes for you',
  '확인하면 좋은 것': 'Worth checking',
  'moncamp 는 이렇게 추천해요': 'What moncamp suggests',
  '한국어': 'Korean',
  '영어': 'English',
  '관련 화면': 'Related screens',
  '공식 원문': 'Official source',
  '정정 이력': 'Corrections',
  '← 게임 업데이트': '← Game updates',
  '조건에 맞는 글이 없어요. 검색어나 분류를 바꿔 보세요.': 'Nothing matches. Try another search term or category.',
  '아직 공개된 게임 업데이트 글이 없어요. 공식 발표를 확인한 글만 올라와요.':
    'No game updates published yet. Only posts confirmed against an official announcement are shown.',
  '그 글을 찾을 수 없어요. 아직 공개되지 않았거나 내려간 글일 수 있어요.':
    'That post could not be found. It may not be published yet, or it may have been taken down.',
  '공식 발표를 확인한 글만 올려요. 모든 변경을 실시간으로 옮기지는 않고, 확인한 주요 변경을 정리해 드려요.':
    'We only publish posts confirmed against an official announcement. We do not mirror every change in real time — we summarize the major ones we verified.',
