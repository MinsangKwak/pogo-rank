...
export function Privacy() {
  ...
  return (
    <div className="page__body" id="page-privacy" data-route="privacy">
      <KoOnlyNote />
      <p>moncamp(이하 "서비스")는 로그인하지 않으면 개인정보를 수집하지 않습니다. Google 로그인은 화면 설정을 계정에 보관하고, 승인된 사용자에게 트레이너 코드를 보여 주는 데 씁니다. 이 문서는 그 과정에서 무엇을 어디에 얼마나 보관하는지 설명합니다.</p>
      <p className="detail__foot">{`시행일 2026-09-04 · 개정 2026-09-06 (이용 통계에 계정 식별자 연결) · 2026-09-07 (플래너 개체 정보 항목) · 2026-09-07 v2.18.0 (처리위탁·국외이전·보유기간·14세·보호책임자·자동수집·셀프 삭제 명시) · ${PRIVACY_VER} v4.2.0 (즐겨찾기·내 포켓몬 기능을 접으면서 그 수집 항목을 뺌). 서비스는 개인이 무료로 운영하며 상업적 목적이 없습니다.`}</p>

      <Sec title="1. 수집하는 개인정보">
        <p>로그인하지 않고 도감·순위표·계산기를 쓰는 경우 개인정보를 전혀 수집하지 않습니다. 아래 항목은 ☰ 메뉴의 "👤 마이페이지" 에서 Google 로그인을 했을 때만 발생합니다.</p>
      ...
      <Sec title="2. 자동으로 수집되는 항목">
        <Bullets items={[
          ...
          '브라우저 로컬 저장소(localStorage) — 패치노트 읽음 여부, 도감 열 수, 마지막으로 보던 탭·모드, 화면 테마(밝게/어둡게), 일정 분류, 통계 켜짐/꺼짐 선택, 약관 동의 버전(pogo_ 로 시작하는 설정값). 브라우저의 사이트 데이터 삭제 또는 ☰ 메뉴 → 통계·저장소 설정 → 캐시 비우기로 지울 수 있습니다',
      ...
      <Sec title="3. 수집 목적">
        <Bullets items={[
          '승인제 운영 — 처음 로그인한 사람을 "승인 대기" 목록에 올리고, 운영자가 승인해야 트레이너 코드가 열립니다',
          '화면 설정 동기화 — 어느 기기에서 로그인해도 같은 화면이 보이도록',
      ...
      <Sec title="4. 처리위탁과 국외 이전">
        ...
        <Table head={['수탁자', '위탁 업무', '이전 항목', '보유·이용 기간']} rows={[
          ['Google LLC — Firebase Authentication · Cloud Firestore', '로그인 처리, 화면 설정·승인 정보 저장', '1의 전 항목', '계정 삭제 시까지 (Firestore 리전: 서울 asia-northeast3, 인증 정보는 Google 글로벌 인프라)'],
      ...
      <Sec title="5. 보유 기간과 파기">
        <Bullets items={[
          '계정 데이터(이메일·이름·사진 URL·uid·화면 설정·동의 기록) — 이용자가 계정을 삭제할 때까지. 삭제하면 Firestore 문서와 Firebase 인증 계정을 즉시 지웁니다',
          'v4.2.0 이전에 저장된 즐겨찾기(★) 도감번호는 기능을 접은 뒤로 더 쓰지도 읽지도 않지만, 이미 저장된 값은 계정 문서에 남아 있을 수 있습니다 — 계정 삭제로 함께 지워집니다',
      ...
      <Sec title="7. 이용자의 권리와 행사 방법">
        <Bullets items={[
          '열람 — ☰ 메뉴의 계정 카드에서 저장된 내용을 그대로 볼 수 있습니다',
          '정정 — 화면 설정(테마)은 화면에서 직접 바꿀 수 있고, 바꾼 값이 계정에 덮어 써집니다',
      ...
      <Sec title="8. 안전성 확보">
        <Bullets items={[
          '접근 제어는 전부 Firestore 보안 규칙(firestore.rules, 저장소에 공개)이 담당합니다 — 본인 문서만 읽고 쓸 수 있고, 승인 전에는 트레이너 코드를 읽을 수 없습니다',
          '운영자는 승인 대기 목록(이메일·이름·uid)만 봅니다. 계정 문서(화면 설정)는 규칙상 운영자도 읽을 수 없습니다',
...
export function Terms() {
  ...
  return (
    <div className="page__body" id="page-terms" data-route="terms">
      ...
      <Sec title="1. 서비스란">
        <p>포켓몬 GO 의 순위표(D-MAX · PvE · PvP), 도감, 일정표, 계산기를 한 화면에서 보는 웹앱입니다. 홈 화면에 설치(PWA)해 앱처럼 쓸 수 있습니다.</p>
        <Bullets items={[
          '무료입니다. 수익을 목적으로 하지 않으며 광고·유료 기능이 없습니다',
          '누구나 로그인 없이 볼 수 있고, 트레이너 코드 열람만 Google 로그인 + 운영자 승인이 필요합니다',
      ...
      <Sec title="2. 계정과 승인">
        <Bullets items={[
          '로그인은 Google 계정만 지원합니다. 처음 로그인하면 "승인 대기" 상태가 되고, 운영자가 승인해야 트레이너 코드가 열립니다',
