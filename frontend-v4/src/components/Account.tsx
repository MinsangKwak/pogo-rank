...
export default function Account() {
  const { enabled, user, status, admin, adminRoot, requestError, recheckError, api } = useAuthStore();
      ...
      return (
        <>
          ...
          <p className="account__sub">
            {'로그인하면 화면 설정이 계정에 저장되고, 승인된 분은 트레이너 코드를 볼 수 있어요. 첫 로그인 때 '}
      ...
      return (
        <>
          ...
          {requestError
            ...
            : <p className="account__sub account__pending">⏳ 승인 대기 중 — 관리자가 승인하면 트레이너 코드를 볼 수 있어요. 관리자에게 알려주세요!</p>}
...
async function remove(setMessage: (text: string) => void) {
  ...
  if (!api || !user) return;
  if (!window.confirm('계정을 삭제하면 저장된 설정과 승인 정보가 함께 지워져요. 되돌릴 수 없어요. 삭제할까요?')) return;
