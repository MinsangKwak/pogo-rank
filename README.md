...
**포켓몬고에서 무엇을 잡고, 어떤 포켓몬을 키울지 정하도록 돕는 서비스.**
다이맥스 티어표 · 맥스 배틀 덱 · 전 종 도감을 중심으로 레이드와 PvP 순위, 육성 플래너, 이벤트 일정을 한 화면에서 봅니다.

| | |
| --- | --- |
| 🌐 **서비스** | **[moncamp.kr](https://moncamp.kr/)** — `deploy` 브랜치 |
| 🧪 **미리보기** | [dev.moncamp.kr](https://dev.moncamp.kr/) — `dev` 브랜치, 버전명에 `-dev` |
| 🧱 **구성** | 서버 없음 · GitHub Pages 정적 호스팅 · 프레임워크 없는 바닐라 JS · PWA(홈 화면 설치 · 오프라인) |
| 🤖 **갱신** | GitHub Actions 가 매일 원본 데이터를 받아 다시 빌드 · 배포 |
| 📦 **의존성** | 파이썬 표준 라이브러리만 (pip 불필요) |

> 포켓몬고 초보 트레이너를 위해 만든 **비영리 · 비공식 팬 프로젝트**입니다. [고지](#고지)를 확인해 주세요.

---

## 빠른 시작

Python 3.10 이상과 `curl` 이 필요합니다. pip 설치도, 가상 환경도 없습니다.

```bash
bash scripts/build.sh      # 원본 데이터를 받아 전체 빌드
open dist/index.html       # 별도 서버 없이 바로 열린다
```

화면 코드만 고쳤다면 `python3 backend/build.py` 하나로 충분합니다. 변경 범위별 빌드 방법은 [개발 문서 §8](docs/DEVELOPMENT.md#8-저장소-구조와-로컬-실행)에 있습니다.

```bash
bash scripts/test.sh                              # 회귀 전체 (27스위트)
bash scripts/test.sh --no-build nav dex-search    # 빌드 생략하고 일부만
```
...
| 문서 | 이런 걸 찾을 때 |
| --- | --- |
| 📘 [개발 문서](docs/DEVELOPMENT.md) | **왜 이렇게 만들었나** — 설계 원칙, 의사결정 기록, 데이터 소스와 파이프라인, 계산식, 프런트엔드 구조, 알려진 한계 |
| 📗 [운영 문서](docs/OPERATIONS.md) | **어떻게 굴리나** — 릴리스 절차, 버전 규칙, 배포 장애 대응, 가입 승인, Firebase 설정, GA4 통계, 점검 목록 |
| 🛡 [인프라 문서](docs/INFRA.md) | **트래픽이 몰리면** — 정적 사이트에서 가능한 남용 대응과 확장 로드맵 (도메인 → Cloudflare → App Check) |
| 🗒 [변경 이력](CHANGELOG.md) | **무엇이 언제 바뀌었나** — 판마다 왜 그렇게 고쳤는지까지 |
...
| 보기 | 무엇을 |
| --- | --- |
| **전체** | pogomate 기준 티어표. 줄을 누르면 선정 근거와 카운터 |
| **딜러** | 보스 타입별 맥스 어태커 순위 |
| **탱커** | EHP(`체력 × 방어 ÷ 받는 피해 배율`) 순위 |

- **⚔️ 이번 주 보스** — 딜러 2 + 탱커 1 추천 파티를 화면 맨 위에서
- **🧩 덱 짜기** — 보스 타입을 고르면 데려갈 셋을 골라 주고, 칸마다 후보를 바꿀 수 있습니다. 짠 덱은 주소로 공유됩니다

### ⚔️ 레이드 · PvP

- **레이드** — 구하기 쉬운 것 중심의 `일반` 과 hawaii 성능표 기반 `전체` 를 전환
- **🧮 솔플 계산기** — 보스를 고르면 난이도 판정과 부활 운영 시뮬레이션으로 혼자 잡을 수 있는지 계산
- **PvP** — 리틀 · 슈퍼 · 하이퍼 · 마스터 리그별 PvPoke 점수와 추천 기술
- **🃏 덱 짜기 · 🧬 개체값 순위** — 메타 추천 덱 3종, 상대 슬롯에 맞춘 커스텀 덱과 카운터, 리그별 IV 랭킹

### 📖 도감

- 전 종 목록 · 이름/번호 검색 · 1~9세대 바로가기 · 줄/카드 보기 전환
- 상세 — 타입, 획득 방식별 CP, 내 개체 CP 계산, 종족값, 기술, 상성, 활용처, 진화. 메가진화와 X · Y 비교표까지
- 아직 안 나온 것은 지우지 않고 **`[미구현]`** 으로 밝힙니다

### 🌱 육성 플래너

도감이 *"이 포켓몬은 얼마나 센가"* 를 보여 준다면, 플래너는 *"내 개체를 어떻게 키울까"* 에 답합니다.

- 보유 개체를 종 · 폼 · 그림자 · 레벨 · 개체값 · 기술 · 육성 상태 · 메모로 계정에 저장
- 같은 종 여러 마리 등록 · CP 자동 계산 · CP 로 레벨 역추정 · 만렙 CP 와 리그 도달 CP 비교
- 상세 화면의 `➕` 로 바로 저장

### 🧰 공통

- **계정** — 관리자 승인제 Google 로그인. 만 14세 이상, 첫 로그인에 약관 · 방침 동의. 계정 삭제는 사용자가 직접
- **일정** — LeekDuck 과 한국 공식 발표를 반영한 월간 일정표, 분류 필터, 기간형 타임라인
- **그 밖에** — 전역 검색, 승인 사용자용 트레이너 코드, 패치 노트, 다크 모드, KR/EN 전환, 오프라인 열람
...
전체 148개 릴리스를 `날짜 → 버전`의 2단계 접이식 목록으로 정리했습니다. 가장 최근 날짜만 기본으로 펼쳐지며, 설명이 한 줄인 초기 버전은 별도의 접이식 영역 없이 표시됩니다. 사용자용 요약은 서비스의 [🎉 패치 노트](https://moncamp.kr/#/release)에서, 변경 배경과 세부 구현 내용은 [변경 이력](CHANGELOG.md)에서 확인할 수 있습니다.

<details open>
<summary><b>2026-09-16</b> — 릴리스 8개 · <code>v3.46.0 … v3.50.0</code></summary>

<details>
<summary><b>v3.50.0</b> · 상세 팝업 재설계 — 요약·배틀 정보·진화 탭 · 같은 창에서 진화 탐색 · CP 계산기 화면 · 하단 고정 버튼</summary>

**한 줄로 쌓던 아홉 섹션을 세 탭과 한 화면으로.** 요약(큰 그림 · CP · [맥스 배틀 | 레이드 | 야생] 포획 CP 세그먼트 · 기술) · 배틀 정보(약점·내성 · 활용 순위 줄 · 보스로 만났을 때 추천 후보) · 진화. 진화 계열이나 추천 후보를 누르면 **같은 창에서** 바뀌고, `[← 돌아가기]` 로 오면 보던 탭·스크롤·계산기 입력값이 그대로다(`DETAIL_NAV` 더미). 하단에 `[포켓몬 도감]`(팝업을 닫고 도감으로) · `[CP 계산기]`(숫자 직접 입력 · ± · 레벨 프리셋)가 고정이고 ✕ 는 늘 머리줄에 있다. PC 는 왼쪽 정보 고정 · 오른쪽 전환의 두 열 팝업 — v2.36.0 오른쪽 패널은 `useDetailPanel() = false` 로 접었다. 곁에서 잡은 버그 하나: 같은 화면을 다시 그릴 때(로그인 판정 끝) `renderPage` 가 열린 팝업을 소리 없이 닫았다 → 주소가 바뀌었을 때만 닫는다. 회귀 `detail.js` 52건으로 다시 씀
...
**흐리게 가려 두면 무엇이 끼어들었는지 읽을 수가 없다.** v3.37.0 의 세 겹 표시(오른쪽 들여쓰기 · 투명도 .4 · blur)를 걷고 **왼쪽 빨간 막대 + 옅은 바탕 + [미구현] 딱지**로 바꿨다. 막대는 `::before` 로 얹는다 — 테두리로 그으면 내용이 밀려 순위 번호가 어긋난다. 그리고 `[미구현]` 을 켜면 표가 **"만약 나온다면" 의 가정판**이 된다: 미구현도 번호를 받고 그만큼 출시분이 밀려, 오늘 1위인 고릴타가 7위로 내려간다. 표 제목에 `[가상 순위]`, 밀린 줄에 `[지금 N위]`. 가정 표에서는 어제 대비 ▲▼ 를 감춘다(두 '움직임' 이 한 줄에서 싸운다). 등급의 100% 기준은 **출시분 1위** 그대로이고, 추천(보스 파티·덱 짜기·솔플)은 여전히 출시분만이다. 회귀 `unreleased.js` 48 → 58건
...
**모바일 리포트의 세 숫자가 전부 4.1초로 같았다** — FCP · LCP · Speed Index 가 같다는 건 "다 받는 시점" 이 병목이라는 뜻이다. 제안 다섯 가지를 다 재 봤다: 주석 10.7KB · 유형별 정리 0 · 컴포넌트화 0(이미 52개 파일이다, 문제는 52개가 **전부 첫 화면에 실린다**는 것) · 민화 8KB · React 는 **−45KB**(오히려 늘어난다). 한 것은 셋 — 배포본에서 주석을 걷어내고(HTML 54곳 + 템플릿 CSS), 번들을 `app.js` 로 빼고(`index.html` **812 → 176KB**, 페이지 소스가 읽을 수 있는 크기가 됐다), 첫 화면이 안 쓰는 94KB(영어 사전 · 패치노트 본문)를 `app-lazy.js` 로 미뤘다. `release.js` 의 94%가 `RELEASE_NOTES` 본문이었다(32.0 → 1.6KB). HTML 에 `<script src>` 로 적지 않는다 — 미리읽기가 대역폭을 나눠 써 FCP 가 되레 1.9s → 3.3s 가 됐던 걸 실측하고 되돌렸다. 결과 **63 → 89(중앙값)**, FCP **2.0s → 0.9s(만점)**, TBT 850 → 180ms. 남은 건 LCP 3.5s 하나이며 `data.js` 분리가 다음 판이다
...
<details>
<summary><b>2026-09-15</b> — 릴리스 18개 · <code>v3.31.1 … v3.45.0</code></summary>

<details>
<summary><b>v3.45.0</b> · 첫 방문 배너를 걷어냈다 — 끄는 길과 고지는 그대로</summary>

**옵트아웃에서 그 배너가 막는 것은 통계가 아니라 방문이다.** v2.18.0 에 동의를 받는 자리로 만들고 v3.39.0 에 "끌 수 있다고 알리는" 자리로 성격을 고쳤지만, 처음 들어온 사람의 첫 화면 아래를 가로막는다는 사실은 그대로였다(제보 — "이걸 팝업으로 띄우니까 아무도 안 들어오는 느낌이야"). `renderConsentBanner()` 와 그 CSS 를 걷어냈다. **없애지 않은 것** — 끄는 길(☰ 메뉴 → 통계·저장소 설정 · 화면 아래 같은 항목), 고지(개인정보처리방침), 이미 끈 사람의 선택(`pogo_consent`, 판정 함수와 저장 키 모두 그대로). 방침에서 배너를 가리키던 문장도 고쳤다 — 문서가 화면보다 뒤처지면 그게 더 나쁘다. `legal.js` 를 뒤집어 "배너가 안 뜬다"·"끄는 길이 두 곳에 남아 있다" 를 지키게 했다(39 → 41검사)

</details>

<details>
<summary><b>v3.44.0</b> · (수정) 보스 추천에 미구현이 끼던 것 · 첫 화면 속도 (CLS 0.911 → 0)</summary>

**추천에 아직 게임에 없는 개체가 1위부터 올라와 있었다.** D-MAX 맨 위 이번 주 보스 아코디언이 `unrel` 을 아예 거르지 않았다 — v3.36.0 이 `DMAX_DATA`·`DMAX_TANK` 에 미구현 행을 넣을 때 이곳만 빠졌다(덱 짜기·상세 추천 칩·대항 칩은 이미 걸렀다). `maxReleased()` 한 곳을 두고 추천 파티·추천 딜러·총계 세 자리에 건다. **체크와 무관하게 출시분만** 쓴다 — 표는 "있으면 이쯤" 이지만 추천은 "오늘 무엇을 데려갈까" 다. 제보의 "미구현을 눌러도 새로고침이 안 된다" 도 같은 원인이었다(체크는 `render()` 를 불렀지만 아코디언이 늘 표 전체를 봤다). 함께 첫 화면 속도도 손봤다 — `<header>` 를 `.layout` 밖으로(부팅 때 `insertBefore` 한 번이 본문을 통째로 밀어 CLS 0.911), `#content:empty { min-height: 100vh }`(푸터가 밀려나며 0.199), `data.js` 를 `JSON.parse` 로(1.4MB 객체 리터럴 파싱 347ms). 로컬 재현 환경에서 Performance **63 → 86**. `preload`(FCP 되레 악화)와 번들 민화(gzip 뒤 1~8KB)는 재보고 **되돌렸다**

</details>

<details>
<summary><b>v3.43.0</b> · 검색 유입 — 제목을 검색어 앞으로 · JS 없이 읽을 본문 · 구조화 데이터 보강</summary>

**제목이 1초 뒤 사라지고 있었다.** `index.html` 의 `<title>` 에 v3.33.0 부터 검색어를 적어 뒀는데, `app-shell.js` 가 홈에서 `document.title` 을 `'moncamp'` 한 마디로 덮어썼다. 구글은 JS 를 돌린 뒤의 제목을 읽으므로 그 문장을 한 번도 본 적이 없다 — 해시 라우팅이라 색인되는 주소는 홈 하나뿐이니 그 하나를 통째로 놓친 셈이다. `SITE_TITLE` 상수로 `<title>` 과 한 글자도 다르지 않게 맞췄다. 제목은 검색어를 앞, 브랜드를 뒤로(`포켓몬고 다이맥스 티어표 · 맥스 배틀 덱 · 도감 | moncamp`) — 30자쯤에서 잘리는데 아직 아무도 모르는 이름에 그 앞자리를 쓰고 있었다. 설명 188자 → 147자, keywords 18개 → 34개(`맥스배틀` 붙여쓰기·`포고` 같은 변형을 넣는다 — 네이버는 토큰을 그대로 맞춰 본다). **네이버 Yeti 는 JS 를 안 돌려** 읽을 본문이 없었으므로 `<noscript>` 에 실제 화면 목록을 적었다(525자 → 1,029자). 구조화 데이터에 `about`(Pokémon GO)·`featureList`·`keywords` 추가. `hreflang`·`SearchAction`·`FAQPage` 는 **넣지 않았다** — 순서대로 언어별 주소가 없고, 조각 주소는 서버로 안 가며, 화면에 FAQ 가 없다

</details>

<details>
<summary><b>v3.42.1</b> · (수정) GA4 측정 ID 교체 — 옛 스트림이 사라져 <code>gtag/js</code> 가 404 였다</summary>

**측정 ID 는 틀려도 화면에서 아무 일도 일어나지 않는다.** 배포에 박힌 `G-8MSZM80JHZ` 로 gtag 라이브러리를 받는 요청이 404 라 히트가 한 건도 안 나가고 있었다 — 아무 ID 나 넣으면 200 이 나오는데 우리 것만 404 였다(옛 blog 스트림 `G-KVRX9FBNDC` 는 200). 스트림이 `moncamp` / `15780045801` / `G-1L6ENS8PVK` 로 바뀌어 있었다. `deploy.yml`·`deploy-dev.yml` 의 `GA_ID` 를 교체하고, 워크플로 주석에 확인 절차(`curl` 로 `gtag/js?id=<ID>` 가 200 인지)를 남겼다. 코드 변경 없음(빌드 입력값·문서만)

</details>

<details>
<summary><b>v3.42.0</b> · 첫 화면 차례 — 기능 타일이 위, 지금 강한 포켓몬이 아래</summary>

**첫 화면의 둘째 칸은 "어디로 갈까" 여야 한다.** `지금 강한 포켓몬`(v3.22.0)이 인사말 바로 밑을 차지하고 있었는데, 아직 실험 중인 화면이라 그만한 자리는 아니다. 차례를 `인사말 → 무엇을 해볼까요? → 지금 강한 포켓몬` 으로 바꿨다. `home.js renderServiceHome()` 의 `append` 인자 차례만 바꾼 변경이다 — `.home-dashboard` 는 `gap` 을 쓰는 그리드라 자식 차례에 기대는 CSS 가 없다

</details>

<details>
<summary><b>v3.41.1</b> · (다듬기) 미구현 줄은 hover 해도 흐린 채로</summary>

**마우스가 스치기만 해도 또렷해졌다.** v3.37.0 은 "살펴보려고 다가가면 읽을 수 있어야 한다" 고 hover 에서 흐림을 풀었는데, 표를 훑는 동안 마우스가 줄을 지나가기만 해도 풀려서 표시한 뜻이 사라졌다(제보 — "지금 너무 명확히 보여"). **hover 에서는 풀지 않는다.** 펼친 근거 줄(`.row__why`)은 형제 요소라 흐림이 안 걸리므로, 눌러서 펼치면 계산식·등급은 그대로 읽힌다. **키보드 포커스(`:focus-visible`)만 예외로 둔다** — 탭으로 옮겨 다니는 사람에게 흐린 줄만 남으면 화면을 쓸 수가 없다

</details>

<details>
<summary><b>v3.41.0</b> · 관리자 권한을 가른다 — 가입 승인은 루트만</summary>

**사람을 들이고 내보내는 일은 주인의 몫으로 둔다.** v3.40.0 은 위임 관리자도 가입 승인을 할 수 있었다 (`admin` 필드만 못 건드렸다). 그걸 좁혔다 — **가입 승인·승인 해제·관리자 지정은 루트만**, 위임 관리자는 `👥 유저 관리` 에서 **목록만** 보고 트레이너 코드 관리와 D-MAX `[미구현]` 보기를 그대로 쓴다. **메뉴 이름이 공 할 수 있는 일이다** — 루트는 `🔑 가입 승인`, 위임은 `👥 유저 관리`. 없는 권한을 이름으로 약속하지 않고, 누르면 규칙이 막는 버튼을 두지도 않는다. **버그 둘도 잡았다** — 관리자 목록에 본인이 두 번 나오며 머릿수까지 하나 더 세어졌고(맨 위 '나' 줄과 목록 줄이 겹쳤다), 루트 계정이 '승인 대기' 에 떴다 — 루트는 allowlist 없이도 쓰는 계정이라 대기자일 수 없다. ⚠️ 보안 규칙을 콘솔에 **다시 게시**해야 한다
...
도메인을 옮기며 GA4 스트림을 정리하다 발견: 저장소 변수 `GA_ID`가 같은 속성의 `blog` 스트림 측정 ID(`G-KVRX9FBNDC`)라 사이트 통계가 처음부터 거기 섞여 들어갔다. *(v3.42.1 — 여기서 적어 넣은 `G-8MSZM80JHZ` 스트림은 그 뒤 사라져 `G-1L6ENS8PVK` 로 다시 교체했다.)* 사이트용 스트림(옛 "pogo-rank 웹", 15750161968 → 이름·URL 을 `moncamp.kr`로 고침)의 `G-8MSZM80JHZ`를 `deploy.yml`·`deploy-dev.yml`에 직접 적는다 — 이 세션에 저장소 변수를 고칠 수단이 없고, 측정 ID 는 배포 HTML 에 그대로 포함되는 공개 식별자라 코드에 둬도 된다. 저장소 변수 `GA_ID`는 더 읽지 않는다. `OPERATIONS.md` 7절 갱신. 코드 변경 없음(빌드 입력값만)
