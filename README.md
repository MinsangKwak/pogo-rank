...
**moncamp는 포켓몬 GO에서 잡거나 키울 포켓몬을 선택하고 배틀을 준비하도록 돕는 서비스입니다.**
다이맥스 티어표와 맥스 배틀 덱, 전체 도감을 중심으로 레이드·PvP 순위와 이벤트 일정을 제공합니다.
...
| **수집 서버** | [`server/`](server/README.md) — Node 22 + Fastify + Postgres (Cloud Run). **화면 표시는 수집 서버의 응답을 기다리지 않습니다** — 중단되어도 기존 화면을 계속 제공할 수 있습니다 |
| **갱신** | GitHub Actions가 매일 원본 데이터를 받아 다시 빌드 · 배포 |
...
원하는 내용으로 이동: [실행 방법](#빠른-시작) · [문서 안내](#문서) · [기능](#기능) · [출처](#데이터-출처) · [버전 이력](#버전-이력)

## 빠른 시작

데이터 빌드에는 Python 3.10 이상과 `curl` 이 필요합니다. Python 패키지 설치나 가상 환경은 필요하지 않습니다.
...
브라우저에서 로그인 이후 화면을 확인하려면 [기여 안내](CONTRIBUTING.md#시작하기)의 로컬 서버 실행 방법을 따르세요. 브라우저 회귀 테스트에는 [Playwright·Chromium 환경](tests/e2e/README.md#전제)이 별도로 필요합니다.

기존 바닐라 화면의 문구만 수정했다면 `python3 backend/build.py` 로 다시 빌드할 수 있습니다. React 화면은 frontend-v4 폴더에서 별도로 빌드해야 합니다. 변경 범위별 빌드 방법은 [개발 문서 §8](docs/DEVELOPMENT.md#8-저장소-구조와-로컬-실행)에 있습니다.
...
| [수집 서버](server/README.md) | 수집 서버의 도입 배경, 집계 보호 장치와 로컬 실행 방법 |
| [수집 서버 스키마](server/SCHEMA.md) · [ERD](docs/server-erd.png) · [API](docs/server-api.html) | 테이블·컬럼·제약 조건, 테이블 관계와 API별 요청·응답 |
...
| 목적 | 기능 |
| --- | --- |
| 잡거나 키울 포켓몬 고르기 | D-MAX 티어표, 레이드·PvP 순위 |
| 포켓몬 정보 확인하기 | 도감, CP 계산, 기술·상성·진화 정보 |
| 키울 포켓몬 챙기기 | 내 포켓몬 — ★ 로 담고 그 종의 일정을 받기 *(실험 기능)* |
| 다음 플레이 준비하기 | 이벤트 일정, 맥스 배틀·PvP 덱 구성 |


<a id="-d-max--다이맥스--거다이맥스"></a>
...
- **이번 주 보스** — 딜러 2 + 탱커 1 추천 파티를 화면 맨 위에서
- **덱 짜기** — 보스 타입을 고르면 추천 포켓몬 3마리를 제안하고, 칸마다 후보를 바꿀 수 있습니다. 구성한 덱은 링크로 공유할 수 있습니다
...
- 미출시 포켓몬은 목록에서 삭제하지 않고 **`[미구현]`** 으로 표시합니다

<a id="-내-포켓몬"></a>

### 내 포켓몬 *(실험 기능)*

도감이 *"이 포켓몬은 얼마나 센가"* 를 보여 준다면, 이 화면은 *"내가 챙기는 포켓몬에게 무슨 일이 생기나"* 에 답합니다.

- **📣 다가오는 소식** — 담아 둔 종에 잡힌 커뮤니티 데이 · 스포트라이트 아워 · 레이드 일정을 가까운 것부터
- **★ 담아 둔 포켓몬** — 일정이 잡힌 줄에는 D-day 가 붙습니다. ★ 를 누르는 자리는 **포켓몬 상세 하나뿐**입니다
- 로그인 전에는 브라우저에 저장하고, 승인된 계정으로 로그인하면 계정 데이터와 **병합합니다** — 기존 즐겨찾기를 유지하기 위해서입니다

> **관리자가 열어 줘야 보입니다.** 아직 다듬는 중이라 실험 기능이며, 옛 주소(`#/plan` · `#/favs` 등)는 이 화면으로 이어집니다.
>
> **개체 기록은 아직 없습니다.** 종 · 레벨 · 개체값 · 기술 · 메모로 내 개체를 저장하는 자리는 개발이 중단된 상태이며 현재 서비스에서는 제공하지 않습니다.
...
- **권한은 두 갈래** — `관리자` 는 **유저 관리**(승인된 사람 목록 · 트레이너 코드), `실험 기능` 은 **내 포켓몬 · D-MAX 의 [미구현] 보기**. 겹치지 않으므로 관리자가 됐다고 실험 기능이 함께 열리지는 않습니다
- **계정에 저장하는 것** — ★ 즐겨찾기 · 화면 테마 · 약관 동의 기록, 그것뿐입니다
...
**포크와 재배포는 허용하지 않습니다.** 소스 코드는 열람과 학습을 위해 공개돼 있을 뿐, 사본을 공개하거나 다른 이름으로 배포·호스팅하거나 복제 서비스를 운영하는 것은 허용하지 않습니다 ([LICENSE](LICENSE)). 데이터와 이미지에는 원출처의 이용 조건이 따로 적용됩니다 — 범위와 조건은 [NOTICE.md](NOTICE.md).
...
전체 231개 릴리스를 `날짜 → 버전`의 2단계 접이식 목록으로 정리했습니다. 가장 최근 날짜만 기본으로 펼쳐지며, 설명이 한 줄인 초기 버전은 별도의 접이식 영역 없이 표시됩니다. 사용자용 요약은 서비스의 [🎉 패치 노트](https://moncamp.kr/#/release)에서, 변경 배경과 세부 구현 내용은 [변경 이력](CHANGELOG.md)에서 확인할 수 있습니다.
...
첫 화면이 새 디자인이고 기본 테마가 다크입니다. 배너에 10월 맥스 배틀 일정 8장이 들어왔고(미공개 보스는 알 그림), 손 화면에서는 배너 · 순위 · 서비스 카드가 짧아지고 도감 목록이 두 줄 카드가 됐습니다. 11월 탱커 팝업의 간격을 넓혔습니다.
...
<details>
<summary><b>2026-09-24</b> — 릴리스 2개 · <code>v5.2.1</code> · <code>v5.2.0</code></summary>

<details>
<summary><b>v5.2.1</b> · 페이지뷰 바로 시행(9/24) · 통계 기본 기간 9/14부터 · 운영 통계를 루트에게만 보이게</summary>

화면 기록을 10월 2일이 아니라 오늘(9/24)부터 시작합니다 — v5.2.0 공지의 날짜를 고치고 정정 항목을 따로 세웠습니다. 운영 통계는 9/14부터 오늘까지를 기본으로 보여 주고, 루트가 아닌 사람에게는 없는 화면으로 보입니다.
...
루트 관리자만 여는 운영 통계 화면(`/admin/stats`)이 생겼습니다 — 가입 · 로그인 · ★ · 검색 · 페이지뷰와 GA4 방문자를 한 화면에서 봅니다. 9월 24일부터 연 화면의 이름을 기록합니다(방침 개정 · 다시 동의).
...
<details>
<summary><b>2026-09-23</b> — 릴리스 3개 · <code>v5.1.0</code> · <code>v5.0.0</code> · <code>v4.9.9</code></summary>

<details>
<summary><b>v5.1.0</b> · 화면 이동 깜빡임 제거 · 홈 배너를 맥스 일정 슬라이드로</summary>

화면을 옮길 때 문서를 다시 열지 않고 새 화면이 준비되면 한 번에 바뀝니다. 홈 배너가 남은 다이맥스 · 거다이맥스 일정을 한 장씩 굴리고, 첫 장이 이번 주 보스(프리져 · 썬더 · 파이어)를 맞게 보입니다.
...
'아래로' 를 눌러 내려간 자리에서 [용도별 상위 포켓몬] 제목이 붙어 다니는 상단 바 뒤에 숨어 있었습니다. 바 높이만큼 미리 띄웁니다. 또 넓은 화면(1440 이상) 전용으로 만든 세로 카드 규칙이 조건 없이 적혀 있어 좁은 화면과 손 화면까지 따라왔습니다. 1440 위에서만 적용되도록 가두어, v4.9.4 가 약속한 대로 좁은 화면은 원래 한 줄 카드로 돌아갑니다.
...
넓은 화면(1440 이상)에서 「다양한 활용처」가 배너 옆 세로 칸으로 올라갑니다. 배너 아래에는 '아래로' 단추가 생겨 [용도별 상위 포켓몬] 으로 내려갑니다. 좁은 화면과 손 화면은 그대로입니다. 도면(스토리북, `/storybook/`)은 관리자 계정으로 로그인했을 때만 열립니다.
...
PC 에서 배너 높이를 600px 로 고정하고 글 2 : 그림 3 으로 다시 짰습니다. 그림은 칸 세로를 다 채우고 날짜·보스 이름은 그림 위에 얹습니다. 손 화면은 그대로입니다.
...
맥스 먼데이·맥스 배틀 데이는 원본에 포켓몬 그림이 없어 소식에 한 건도 걸리지 않았습니다. 영문 제목에서 종을 읽어 도감 이름표에 있는 것만 잇습니다.

</details>

<details>
<summary><b>v4.9.0</b> · 화면·약관·문서 한국어 문구 정리</summary>

화면·약관·저장소 문서의 한국어를 한 결로 다듬었습니다. 기능 변경은 없습니다. 규칙 대조에서 영문 사전이 짝을 잃은 줄 셋과 규칙 파일의 섞인 말투를 함께 고쳤습니다.
...
LeekDuck 발표를 바탕으로 월 일정(5성·메가·섀도우 레이드, 맥스 먼데이, 레이드 아워·스포트라이트)을 자동으로 채웁니다. 같은 일정에 수동 항목이 있으면 이를 우선합니다. 즐겨찾기 1건의 저장 실패로 다른 즐겨찾기까지 사라지던 문제도 수정했습니다.
...
백업 상태를 확인할 수 없을 때 성공으로 처리하던 문제를 수정했습니다. 서버 연결 실패나 오류 응답이 있으면 해당 점검 단계를 실패로 처리합니다. 화면에 보이는 변화는 없습니다.

</details>

<details>
<summary><b>v4.8.5</b> · 백업 사고가 시간이 지나면 지워지던 것을 막았습니다</summary>

한 컬렉션이 비었을 때, 망가진 백업이 몇 판 쌓이면 성한 판이 비교 범위 밖으로 밀려나 판정이 저절로 괜찮아졌습니다. 비교 기준을 "여태까지의 최대" 로 바꿔 시간이 지나도 안 지워지게 했고, 진짜로 줄어든 경우만 사람이 직접 끊도록 했습니다. 한 번 끊은 뒤 수가 되살아나면 기준도 따라 올라가, 그 다음 사고를 옛 기준이 덮지 않습니다. 화면에 보이는 변화는 없습니다.

</details>

<details>
<summary><b>v4.8.4</b> · 백업 재시도가 막히던 것과, 손실이 일주일 뒤 조용해지던 것을 고쳤습니다</summary>

백업을 같은 날 다시 돌리면 GitHub 아티팩트 이름이 겹쳐 재시도 자체가 막혔습니다. 그리고 한 컬렉션이 비었을 때, 그 다음 주에는 "지난주도 0이었으니 괜찮다" 가 되어 조용해졌습니다. 이제 최근 다섯 판과 견주어 고쳐질 때까지 계속 알립니다. 화면에 보이는 변화는 없습니다.

</details>

<details>
<summary><b>v4.8.3</b> · API 설명서를 반쪽만 고쳤던 것을 마저 고쳤습니다</summary>

바로 앞 판에서 고친 문법 오류가 저장소 파일에만 반영되고, 정작 돌아가는 서버가 내보내는 문서에는 그대로 남아 있었습니다. 이제 서버가 내보내는 문서를 그대로 받아 저장소에 넣고, 검사도 그 주소를 직접 봅니다. 화면에 보이는 변화는 없습니다.
...
수집 서버의 순위 문턱과 12개월 파기에서 셋을 더 고쳤습니다. 화면에 보이는 변화는 없습니다.

</details>

<details>
<summary><b>v4.7.2</b> · 방침의 IP 설명을 바로잡았어요</summary>

코드 리뷰가 셋을 잡아 고쳤습니다. IP 주소를 저장하지 않는 건 그대로지만, 요청이 쏟아지는 걸 막으려고 받는 순간 잠깐 보기는 해서 "아예 안 본다" 고 적었던 문장을 고쳤습니다. 방침에 약속한 12개월 파기를 실제로 구현했고, 순위의 사람당 한도가 날짜별로 걸리도록 맞췄습니다.
...
GA4 가 돌려주지 않는 원본 이벤트를 남길 서버(`server/`)를 세웠습니다. 화면에 보이는 변화는 없고, 검색 기록이 GA4 와 내 DB 에 나란히 쌓이기 시작합니다. 화면은 이 서버를 런타임에 읽지 않으므로 서버가 중단되어도 기존 화면을 계속 제공할 수 있습니다.
...
<details>
<summary><b>2026-09-20</b> — 릴리스 19개 · <code>v4.6.3</code> · <code>v4.6.2</code> · <code>v4.6.1</code> · <code>v4.6.0</code> · <code>v4.5.11</code> · <code>v4.5.10</code> · <code>v4.5.9</code> · <code>v4.5.8</code> · <code>v4.5.7</code> · <code>v4.5.6</code> · <code>v4.5.5</code> · <code>v4.5.4</code> · <code>v4.5.3</code> · <code>v4.5.2</code> · <code>v4.5.1</code> · <code>v4.5.0</code> · <code>v4.4.2</code> · <code>v4.4.1</code> · <code>v4.4.0</code></summary>

<details>
<summary><b>v4.6.3</b> · 포켓몬 검색순위 보류</summary>

실측(최근 7일 방문 54명)으로는 순위가 서지 않아 홈 검색 보드와 `#/hot` 화면, 집계 스크립트를 내렸습니다. 검색 기록(GA4 `search`)은 계속 쌓이고, 구현은 `ranking` 브랜치에 보존했습니다.
...
검색순위 전체 보기의 나라 고르기에 검색이 집계된 나라만 나오고, 지도도 그 나라만 눌립니다. dev 미리보기 샘플에 나라 셋을 실어 모양을 확인합니다.
...
하루치 검색이 문턱에 못 미치면 일주일 창으로 넓혀 순위를 세우고, 화면은 "최근 일주일 동안" 으로 말을 바꿉니다. 집계 로그에 최근 7일 방문 규모를 남겨 `0건` 의 원인을 가릅니다.
...
홈 왼쪽에 포켓몬 검색순위, 오른쪽에 다음 맥스 배틀 포스터(날짜·보스는 일정에서). 전체 보기에는 세계 지도가 생겨 나라별 순위를 봅니다. 몇 번 안 열린 포켓몬은 순위에 안 서도록 문턱을 뒀습니다. 제목은 세는 범위에 맞춰 '포켓몬 검색순위' 입니다.
...
'용도별 상위 포켓몬' 뒤에 있던 것을 본문 맨 위(히어로 바로 아래)로 올렸습니다.
...
창이 최근 24시간이고 GA4 가 이벤트를 몇 시간 늦게 처리해 '오늘' 은 지키지 못할 약속이었습니다. 미리보기 샘플도 넷으로 고정해 빌드마다 줄이 바뀌지 않게 했습니다.
...
두 열 폭에 `CP 100% 1,260` 이 한 줄로 안 들어가 라벨이 접히고 숫자까지 `1,26 / 0` 으로 갈라졌습니다. 라벨을 위로 올려 숫자에 카드 폭을 통째로 주고, 숫자에는 자릿수가 갈리지 않도록 줄바꿈을 막았습니다.
...
dev 에는 GA 조각이 없어 아무리 검색해도 쌓이지 않습니다. 고른 이름을 이 브라우저에 세어(`pogo_hotsearch_local`) 미리보기에서만 '내 검색 · 이 브라우저' 딱지와 함께 세웁니다. 고르는 자리가 제대로 잡히는지를 운영에 올리기 전에 확인하려는 장치입니다.
...
dev 는 GA 로 아무것도 안 보내 표가 늘 비고, 그러면 화면이 구역을 안 그려 만든 것을 볼 수 없었습니다. `BUILD_CHANNEL=dev` 일 때만 실제 순위표 상위 이름으로 열 줄을 채우고 '미리보기 샘플' 딱지를 답니다. 운영으로 새지 않는다는 것은 워크플로를 읽는 검사 둘이 지킵니다.
...
사람들이 무엇을 검색하는지 보려고 GA4 표준 `search` 이벤트를 보냅니다. 도감·솔플·PvP 덱·개체값·게임 업데이트 다섯 검색창에서 타이핑이 멎고 1.5초 뒤 한 번만 기록하고, 통계 동의를 끈 사람은 보내지 않습니다. 화면 변화가 없어 패치 노트에는 적지 않았습니다.

</details>

<details>
<summary><b>v4.5.3</b> · 잉크 선 한 번 더 반토막 — 기본 0.5px · 카드 1px</summary>

윤곽선이 은은하게 형태만 잡도록 `--ink-w` 를 반으로 더 줄였습니다.
...
일러스트의 알파 실루엣을 drop-shadow 네 방향으로 둘러 윤곽선을 만듭니다. 기본 2px, 큰 그림은 4px. 색은 `--ink` 변수 하나라 다크 모드에서는 밝은 선으로 뒤집히고, 어두운 판(홈 히어로)은 밝은 선을 씁니다.

</details>

<details>
<summary><b>v4.5.0</b> · 포켓몬 그림을 공식 일러스트로 — 도감·순위·레이드·홈 전부</summary>

정지 그림의 출처를 96×96 도트에서 공식 일러스트(475px → 256px 축소, 256색 양자화)로 바꿨습니다. id 가 도트와 같아 1,184장 전부 매핑 없이 대응되고, 파일명이 그대로라 프런트는 렌더링(`pixelated` 해제)만 바뀌었습니다. 장당 평균 1KB → 11KB, 원본 크기는 배포하지 않습니다. CI 캐시의 옛 도트가 되살아나지 않도록 형식 표식(`.format`)을 두고, 일러스트가 없는 id 는 도트로 폴백합니다. 움직이는 도트 GIF 는 일러스트를 덮어 기본을 끔으로 바꿨습니다(설정에서 켤 수 있습니다).
...
**먼저 쟀다 — Lighthouse 넉 장 · 긴 작업 · 레이아웃 이동의 출처 · 메인 청크의 구성.** D-MAX 의 CLS 0.85 는 머리 단추 줄이 데이터 뒤에 끼어들며 제목을 56px 밀던 것이었고(→ 0.05), 긴 작업(50ms+)은 넉 장 모두 0 이 됐다.

- **자리를 미리 지킨다** — 단추가 서는 화면은 빈 줄 높이를, 스플래시는 한 화면을(바닥글이 먼저 보였다가 밀리지 않게).
- **홈 히어로는 데이터를 안 기다린다** — 아래 추천만 `<Suspense>` 로. 셸이 기술 변경 한 줄 때문에 gameday.json 을 기다리던 것도 기다리지 않는 훅으로.
- **카드 100장은 `content-visibility: auto`** — 화면 밖은 건너뛴다. 검문 둘은 훑기 전에 잠시 켠다(안 켜면 화면 밖 카드가 그물 밖이다 — 주입해 확인).
- **데이터 미리 받기 · react 청크 분리(main 367 → 157KB)** — 운영 CDN(HTTP/2)에서 같은 조절로 dev 대 운영을 대조해 확인: 레이드 −764ms · 도감 −493ms · D-MAX −128ms · 홈 같음. `scripts/check_perf.mjs` 가 그 대조 도구다.
...
**제보(사진) — dev 알 부화 카드의 포켓몬이 카드를 꽉 채운다.** 원본 37px 그림이 150px 로 그려지고 있었다. `fitZoom()` 이 확대를 2배로 막으려고 넣는 padding 이 `content-box` 라 상자를 키우기만 했다 — `catalog.css` 그리드 그림에 `box-sizing: border-box` 한 줄로 2배에서 선다.
...
**dev 에 올라온 두 커밋(17파일 · 603줄)을 `CLAUDE.md` 에 대조해 고친 판이다.** 디자인 의도는 그대로다 — 홈의 잡지 목차식 안내와 맥스 배틀 장면, 도감·순위·레이드 카드의 바닥 무대, 검색식 콘솔, 달력 월 이동과 10월 일정 6건.

**명암비 검문이 먼저 잡았다.** `#fff` 글자가 `--brand` 채움 위에 놓인 자리 여섯이 다크에서 **2.29** 였다. "채움 위 글자는 `--bg`"(§1-b) 그대로 고쳤다. 정적으로는 hex 색 28 · `#fff` 글자 8 · 격자 밖 글자 크기 28 · 줄높이 16 · 간격 52 · 44px 미만 타깃 3 · 영어 주석 18 을 전부 0 으로.

**어두운 판 토큰 `--plate` 한 벌.** 검색식 콘솔과 홈 히어로처럼 테마를 안 타는 자리 — `--bg`·`--fg` 는 다크에서 뒤집히고 hex 는 짝이 끊긴다.

**규칙을 맞추다 낸 사고 하나.** `#page-head` 의 `margin` 단축이 왼쪽 값(사이드바 자리)까지 지워 본문이 메뉴 밑으로 들어갔다 — v4.3.4 와 같은 모양. 세로 longhand 로.

**되살린 것.** 「⚔️ 기술 변경」 메뉴 줄(`#menu-changes`, §2) · 홈 장면의 이름을 도감 이름표에서(§3) · `.row__portrait` 래퍼로 죽은 `.row > .sprite` 선택자 다섯.

**접두사 `html body #root`** 는 v3 스킨을 이기는 v4 의 장치 하나로 통일했고, 파일 안의 자기 덮어쓰기를 합쳤다. v3 스킨을 걷어내는 일은 남은 정리 항목이다.

검사 57건 · 화면 46장 `NaN` 없음 · 명암비 42장 0(전 6) · 전후 스크린샷 24장 대조.
...
<details>
<summary><b>2026-09-19</b> — 릴리스 12개 · <code>v4.3.9</code> · <code>v4.3.8</code> · <code>v4.3.7</code> · <code>v4.3.6</code> · <code>v4.3.5</code> · <code>v4.3.4</code> · <code>v4.3.3</code> · <code>v4.3.2</code> · <code>v4.3.1</code> · <code>v4.3.0</code> · <code>v4.2.5</code> · <code>v4.2.4</code></summary>
...
<details>
<summary><b>2026-09-18</b> — 릴리스 8개 · <code>v4.2.3</code> · <code>v4.2.2</code> · <code>v4.2.1</code> · <code>v4.2.0</code> · <code>v4.1.1</code> · <code>v4.1.0</code> · <code>v4.0.1</code> · <code>v4.0.0</code></summary>
...
주소도 화면도 그대로이고, 바뀐 것은 그리는 방법입니다. 화면을 옮길 때 본문을 통째로 다시 그리지 않아 이동이 덜 끊기고, 첫 화면에서 받는 양이 줄었습니다. 화면 스물하나와 로그인·영어·PWA 를 옮겼고, 클래스 이름·DOM id·저장 키·GA 이벤트명·Firestore 문서 모양은 그대로 두었습니다.
...
<details>
<summary><b>2026-09-17</b> — 릴리스 9개 · <code>v3.56.0 … v3.61.2</code></summary>

<details>
<summary><b>v3.61.2</b> · (긴급) 게임에 없는 폼이 도감 검색에 나오던 것</summary>

제보: `자마젠타` 검색에 **[거다이맥스 방패왕 자마젠타]** 가 떠 있었다 — 게임마스터에만 있고 게임에는 없는 폼이다. 같은 방식으로 16종이 새고 있었다. D-MAX 화면은 [미구현] 을 관리자에게만 여는데 검색 색인만 그 문을 안 잠갔다. `buildSearchIndex()` 가 행의 `unrel`·`released:false` 를 버리고 있었고, 검색 결과의 미구현 딱지는 **원종 도감번호**로 판정해 폼 줄에는 안 붙었다. 색인이 표시를 들고 오게 하고 `searchVisible()` 한 곳에서 거른다 — 도감 검색·개체값 순위·[개체 추가] 가 모두 지난다. 관리자는 그대로 보고, 딥링크는 계속 열린다. 회귀 5건 추가.

</details>

<details>
<summary><b>v3.61.1</b> · 합친 뒤 남은 옛 자리 셋</summary>

v3.61.0 이 화면의 뼈대를 ★ 로 바꿨는데 그 값을 읽던 세 자리가 옛 것을 가리키고 있었다. ☰ 메뉴 계정 카드가 개체 기록을 세어 담아도 0마리였던 것(제보)을 ★ 수로 고치고 눌러 `#/planner` 로 가게 했으며, `toggleFav()` 가 열려 있는 카드를 그 자리에서 다시 그린다. 상세의 [＋ 내 포켓몬] 은 같은 스위치에 묶어 내렸다 — 저장은 되는데 볼 화면이 없었다. 옛 `#/favs` 는 도감이 아니라 `#/planner` 로 잇는다. 회귀 4건 추가.

</details>

<details>
<summary><b>v3.61.0</b> · 육성 플래너·내 포켓몬을 한 화면으로 — 뼈대를 ★ 즐겨찾기로</summary>

개체 기록은 일곱 번 입력해서 CP·리그 도달·유사백을 얻었는데 셋 다 계산기가 저장 없이도 알려 주던 값이고, 게임과 동기화되지 않아 사탕을 먹이는 순간 틀려졌다. ★ 담기는 탭 한 번에 📣 일정을 돌려준다 — 그쪽을 뼈대로 세우고 두 화면(`planner` · `planner-collection`)을 `🎒 내 포켓몬` 하나로 합쳤다. 옛 주소 셋은 legacy 로 잇는다. 화면 순서가 뜻이다: **일정이 먼저, 목록이 나중** (v3.4.0 에 ★ 를 걷어낸 이유가 "목록이 도감을 이름순으로 거른 것과 다르지 않아서" 였다). 📣 소식은 실험 기능 문을 열어 담을 수 있는 사람이면 다 본다(승인 대기 포함). 개체 기록은 지우지 않고 `PLAN_MONS_ENABLED` 만 내렸다. 회귀 59건으로 다시 씀 + 다섯 스위트 이관.

</details>

<details>
<summary><b>v3.60.0</b> · ★ 즐겨찾기 부활 + 📣 담아 둔 포켓몬의 일정 배지</summary>

v3.4.0 에 ★ 즐겨찾기를 걷어낸 이유가 "담은 뒤에 할 일이 없어서" 였다. 담은 뒤를 만들었다 — 담아 둔 포켓몬이 커뮤니티 데이·스포트라이트 아워·레이드 보스에 뜨면 상세에 📣 배지가 서고, 누르면 이벤트 일정으로 간다. 담는 곳은 **상세 하나뿐**이고, 배지는 **실험 기능 참가자**에게만 보인다. 이벤트의 `extraData` 이미지 주소에서 도감번호를 긁어(두 형식 모두) 포켓몬이 걸린 이벤트가 7 → 13건으로 늘었고, 결과는 `FAV_EVENTS` 로 코어 데이터에 2.6KB 실린다. 관리자 화면에 `🧪 실험 기능` 버튼을 달아 루트가 계정별로 켠다. 승인 대기 계정도 즐겨찾기까지는 저장된다(보안 규칙 `favsOnly()`). 더불어 팝업 안의 버튼을 한 벌로 맞췄다 — 같은 테두리·모서리·그림자에 44px, 색이 차는 것은 누를 것 하나뿐. 회귀 31건 신설 + 팝업 버튼 계약 11건.

</details>

<details>
<summary><b>v3.59.0</b> · 잠금을 육성 플래너·내 포켓몬 둘로 좁힌다</summary>

잠금 장치는 이미 있었으나 임시 스위치(`LOCK_OPEN_ALL`)가 전부 무효화하고 있었다. 스위치를 끄고 잠그는 화면을 **내 것을 적어 두는 두 화면**으로 좁혔다. 읽기만 하는 화면은 열어 둔다 — 잠그면 처음 온 사람이 볼 것이 없어져 가입할 이유도 사라진다. 잠시 써보기는 껐다(코드는 남기고 스위치만).

</details>

<details>
<summary><b>v3.58.0</b> · 솔플 레이드 — 복합 타입 보정을 때리는 타입으로</summary>

복합 타입 보스 보정을 포켓몬의 타입이 아니라 **실제로 쓰는 기술 타입**으로 잰다(570 중 238 행이 바뀐다). 섀도우 샹델라는 고스트/불꽃인데 기술이 불꽃뿐이라 에스퍼 보정을 못 받는데, 옛 코드는 얹어 줘 화력을 60% 높게 잡고 1위로 추천했다. 두 기술 타입은 한 사이클의 피해량으로 가중한다. 자기 타입과 같은 값은 싣지 않아 +9KB 로 줄였다. 회귀 14건 신설.

</details>

<details>
<summary><b>v3.57.1</b> · 집계 호스트를 moncamp.kr 로 좁힌다</summary>

GA 보고서에 다른 사이트의 글 제목이 섞여 있었다. 호스트로 갈라 보니 옛 배포 주소와 남의 사이트가 같이 얹힌 `minsangkwak.github.io` 였다. 옛 주소는 이미 301 이라 스니펫의 호스트 조건에서 `github.io` 를 뺀다.

</details>

<details>
<summary><b>v3.57.0</b> · 화면별 측정 — 해시 라우트를 가상 경로로</summary>

3일치 실측에서 조회수 60 이 전부 경로 `/` 하나로 뭉쳐 있었다. 해시 라우터라 `page_view` 가 세션당 한 건뿐이었고, 해시는 pathname 이 아니라 경로도 `/` 였다. 화면을 옮길 때마다 `page_view` 를 보내고 주소를 가상 경로로 바꾼다. 죽어 있던 진입 경로(`from`)도 클릭 캡처로 되살렸다. 회귀 15건 신설.

</details>

<details>
<summary><b>v3.56.0</b> · 노트 테마 정리 — 셸 분리 · 토큰화 · 덮어쓰기 걷어내기</summary>

codex 판 15개가 들여온 회귀 13개를 되돌리고, CSS 를 **변수화 · 덮어쓰기 최소화 · 컴포넌트화** 기준으로 정리했다. 홈 파일에 섞여 있던 앱 셸 규칙 20KB 를 `components/shell-notebook.css` 로 분리하고, 바탕색과 열 사다리를 토큰 한 곳에서 받게 했다. 서비스워커의 `no-cache` 와 `rem` 테두리 90곳도 함께 고쳤다. 노트 테마의 생김새는 의도된 디자인으로 보고 회귀를 새 화면에 맞췄다.
...
<details>
<summary><b>2026-09-16</b> — 릴리스 19개 · <code>v3.46.0 … v3.55.1</code></summary>

<details>
<summary><b>v3.55.1</b> · 홈 머리줄 — 제목·버튼 한 줄, 부제는 아래 줄</summary>

부제 길이에 따라 구역마다 머리줄 모양이 달랐다. 부제를 늘 제 줄에 놓고(`flex-basis: 100%`) 버튼을 제목 줄로 올려(`order`) 세 머리줄을 한 모양으로 맞췄다. 읽는 차례는 그대로 두고 보이는 자리만 옮긴다.

</details>

<details>
<summary><b>v3.55.0</b> · 홈 — 게임 업데이트를 맨 아래 칸으로</summary>

**소식은 먼저 읽을 것이 아니다.** 홈 순서를 `소개 → 기능 → 용도별 상위 포켓몬 → 게임 업데이트` 로 바꿔 처음 온 사람이 서비스가 무엇인지부터 읽게 했다. 자리만 내리면 격자 간격에 묻혀 구역이 끝났다는 신호가 사라져, 위쪽에 가로줄을 하나 그었다. 회귀가 `.home-dashboard` 의 마지막 자식을 확인한다.

</details>

<details>
<summary><b>v3.54.0</b> · 게임 업데이트 — 아카이브 색인(인용) · 기사와 한 목록 · 과거 백필 모드</summary>

**공식 사이트는 최근 30건만 내놓는다**(사이트맵·API·페이지네이션 없음 — 실측). 그래서 과거는 두 층으로 푼다: 사람이 검증해 쓴 **기사**와, 공식 제목·날짜·원문 링크에 **원문에서 그대로 따온 인용** 한 문단만 붙인 **아카이브**. 인용은 우리 문장이 아니라 검증 없이 내보내도 지어낸 내용이 없고, 요약이 붙으면 그 자리가 기사가 된다(승격 — 빌드가 중복 아카이브를 뺀다). 지금 닿는 범위로 아카이브 32건을 채워 목록이 43건. 아카이브는 `data-lazy.js` 에 실어 첫 화면을 건드리지 않는다. 2020년 백필은 `--backfill 20200101`(웹 아카이브 CDX) 로 붙였으나 **개발 컨테이너의 이그레스 정책에 막혀 실제 동작은 러너에서 확인해야 한다.** API 는 두지 않았고 수집기는 파이썬 유지. 회귀 62건

</details>

<details>
<summary><b>v3.53.0</b> · 게임 업데이트 — 수집 자동화 · 글 11건 · 지난 소식 더 보기</summary>

**수집은 자동, 판단은 사람.** `scripts/collect_game_updates.py` 가 공식 뉴스 색인과 릴리스 노트를 훑어 후보를 모으고(`game_update_candidates.json`), 매일 06:00 KST 워크플로가 **PR 로** 올린다 — 빌드는 그 파일을 읽지 않으니 자동 게시는 일어나지 않는다. 실제 변경은 뉴스가 아니라 **공식 릴리스 노트·알려진 문제**에 쌓여 있어, 거기서 8건을 확인해 옮겨 총 11건. 그 문서는 발표일이 없어 공개 조건을 `announcedAt` 또는 `checkedAt` 으로 넓히고 목록에 `확인` 날짜를 적는다. 목록은 5건씩 `[지난 소식 더 보기]` 로 과거를 잇는다. 회귀 51건

</details>

<details>
<summary><b>v3.52.0</b> · 게임 업데이트 — 체육관 글 추가 · moncamp 는 '추천' 을 적는다 · 원문 미리보기 카드</summary>

**체육관 글**: 「또 다른 메가톤급 소식!」(공식, 한국어·영어 대조)에서 "체육관을 지키는 포켓몬과의 체육관 배틀에서는 추가 스페셜 어택을 사용할 수 없습니다" 를 확인해 첫 체육관 글로 냈다. 첫 제보였던 방어 시스템 변경은 다른 사안이라 여전히 `draft`. **moncamp 축**: `점검 필요/반영 완료` 는 우리 쪽 작업 상태라 걷어내고 `moncampAdvice`("moncamp 는 이렇게 추천해요") 한 문단으로 — 상태 알약은 근거·게임 적용 두 축. **원문 카드**: 줄 링크를 썸네일 + 제목 + 주소 카드로. `og:image` 원본이 2.7MB GIF 라 호스트 크기 옵션으로 25KB JPEG 까지 줄이고, 받지 못하면 그림 자리를 지운다. 회귀 45건

</details>

<details>
<summary><b>v3.51.0</b> · 📢 게임 업데이트 신설 — 게임 쪽 변경을 모아 읽는 상설 창구 (1단계)</summary>

**게임이 바뀐 일 / 언제 무엇이 열리는지 / moncamp 가 바뀐 일을 세 화면으로 갈랐다.** 편집 원본은 `backend/config/game_updates.json` 한 벌이고, 빌드가 검증한 뒤 `editorialStatus='published'` 인 글만 번들에 싣는다 — 확인 대기 글은 주소를 알아도 열 수 없다. 상태는 세 축(근거 / 게임 적용 / moncamp 반영)으로 따로 적고, 이전 값을 모르면 `이전 값 미확인` 이라고 적는다. 목록(`#/game-updates`)은 주요 변경·검색·분류·기간, 상세(`#/game-updates/<id>`)는 핵심 → 전후 → 영향 → 확인 → 반영 → 관련 화면 → 공식 원문. 홈에 주요 소식 3건. 첫 글 두 건은 공식 원문을 직접 받아 대조했고, 제보였던 체육관 방어 변경은 원문을 특정하지 못해 `draft` 로 남겼다. 곁들여 상세 팝업의 `#번호`·폼 배지 세로 정렬을 맞췄다. 회귀 `game-updates.js` 36건 신설

</details>

<details>
<summary><b>v3.50.5</b> · 접힌 머리 순서 — [타입][#번호] · 이름 · 영문명</summary>

접힌 머리도 큰 머리와 같은 순서 — 첫 줄 배지 + 번호, 둘째 줄 이름, 셋째 줄 영문명. 영어 화면에서는 그 줄이 한글 이름(`data-i18n="alt"`) — Metagross 가 두 번 서지 않는다

</details>

<details>
<summary><b>v3.50.4</b> · 접힌 머리 — 진화 탭도 작게 · 타입 배지는 이름 왼쪽에</summary>

배틀 정보 · 진화 · 계산기가 같은 접힌 머리(작은 그림 + `[드래곤][비행] 망나뇽 #0149`). 배지는 이름 왼쪽에 한 벌 더 두고 CSS 가 `data-tab` 으로 고른다

</details>

<details>
<summary><b>v3.50.3</b> · PC 상세 팝업 높이 상한 800px</summary>

`height: min(86vh, 80rem)` — 큰 모니터에서 화면을 따라 끝없이 커지지 않는다

</details>

<details>
<summary><b>v3.50.2</b> · (수정) 상세 팝업 ✕ 를 머리줄 버튼과 같은 줄·간격으로</summary>

✕ 가 카드 테두리 2px 만큼 위에 떠 있었고 오른쪽 간격도 넓었다 — 같은 줄·6px 간격으로

</details>

<details>
<summary><b>v3.50.1</b> · 상세 팝업 — 머리줄 [내 포켓몬][포켓몬 도감] · 하단 [링크 복사][CP 계산기] · 타입 배지는 그림 모서리로</summary>

**자리의 뜻을 갈랐다.** 머리줄은 팝업 밖으로 가는 것(포켓몬 도감 · 닫기)과 다른 화면에 남기는 것(＋ 내 포켓몬), 하단은 이 포켓몬에 대한 동작(링크 복사 · CP 계산기). 타입 배지는 v2.35.0 자리(그림 왼쪽 위 모서리)로 되돌렸다
