...
/**
 * `/mon/<스프라이트 id>` 의 본문. **팝업이 아니라 페이지다** (v5 Phase 6).
 * 공유 링크에는 이름이 없다 — 스프라이트만 넘기고 상세가 색인에서 찾게 둔다
 * (v3 openDetailBySprite 와 같은 길)
 */
function MonPage({ rest, onOpen }: { rest: string; onOpen: OpenMon }) {
  const sprite = Number(rest);
  if (!Number.isFinite(sprite) || !sprite) return <Home onOpen={onOpen} />;
  return <MonDetail pick={{ sprite }} inline />;
}
...
function Screen({ route, rest, onOpen }: { route: RouteDef; rest: string; onOpen: OpenMon }) {
  ...
  switch (route.id) {
    ...
    // 모달보다 페이지가 맞다 — 뒤로 갈 곳이 없는데 ✕ 만 있는 화면이었다
    case 'mon': return <MonPage rest={rest} onOpen={onOpen} />;
...
export interface AppProps {
  /**
   * 서버가 미리 그려 둔 덩어리. **데이터를 받는 동안 이 자리에 선다** (v5 Phase 6).
   *
   * 전에는 여기가 언제나 <Splash/> 였다. 그러면 크롤러가 받는 문서의 본문이
   * '불러오는 중…' 한 줄이고, 그것이 포켓몬 1,100종이 검색에 한 페이지로 잡히던 이유다.
   * 이제 상세 주소에는 그 포켓몬의 사실이 미리 그려져 온다 (components/MonFacts.tsx).
   */
  seo?: ReactNode;
}

export default function App({ seo }: AppProps = {}) {
  ...
  // 주소가 바뀌면 팝업도 따라간다.
  //   /mon/<스프라이트 id> 로 바로 들어오면 본문이 상세다 — 팝업은 안 띄운다,
