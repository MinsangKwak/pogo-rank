...
export const usePrefStore = create<PrefState>((set, get) => ({
  ...
  setTheme: (next) => {
    ...
    set({ theme: next });
    // v5 Phase 6/7 — 계정에 남기지 않는다. 아래 주석 참고
  },
...
// 첫 그림 전에 <html data-theme> 을 맞추는 일은 **문서 머리의 작은 스크립트**가 한다
// (app/layout.tsx 의 THEME_SCRIPT). v5 Phase 6 에 옮겼다 —
// 여기서 하면 서버에서도 실행되는데 그쪽에는 document 가 없다.
// 머리 스크립트가 더 이르기도 하다: 번들을 받기 전에 돈다

/**
 * 계정에 테마를 남기던 자리 — **v5 Phase 6/7 에 없앴다.**
 *
 * Firestore 의 users 문서는 칸이 자유로워 테마 하나를 슬쩍 얹을 수 있었다. 관계형에서는
 * 칸을 만들어야 하는데, **만들 만한 값이 아니다** — 테마는 기기마다 다른 것이 자연스럽고
 * (직장 모니터는 밝게, 침대에서는 어둡게), 실제로 기기 간에 맞춰 달라는 요청도 없었다.
 * 이 기기에만 남긴다 (`pogo_theme`). 필요해지면 그때 칸을 만든다.
 */
