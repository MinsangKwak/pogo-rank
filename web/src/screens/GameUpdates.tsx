...
import { routeById, routeHref, type RouteId } from '../routes';
...
function Back() {
  return <button className="upd__back" onClick={() => { go(routeHref('game-updates')); }}>← 게임 업데이트</button>;
}
          ...
          <div className="upd__related">
            {article.related.map((routeId) => (
              <button key={routeId} className="uchip"
                onClick={() => { go(routeHref(routeId as RouteId)); }}>
