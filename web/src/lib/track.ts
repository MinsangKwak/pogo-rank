...
export function gaVirtualUrl(): string {
  const raw = location.pathname.replace(/^\/+/, '').split('?')[0]?.replace(/\/+$/, '') ?? '';
...
// 콘솔에서 맞춤 측정기준을 따로 등록하지 않아도 Data API 로 바로 읽힌다.
// 순위 화면은 v4.6.3 에 내렸지만(ranking 브랜치) 기록은 계속 쌓는다 — 다시 올릴 때 이 수가 밑천이다.
