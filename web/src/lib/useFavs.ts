...
//   로그인 전  이 브라우저(pogo_favs) — 로그인하는 순간 계정으로 합쳐 올린다 (AuthBridge)
//   로그인 후  계정 (`/v1/me/favorites`) — v5 Phase 5 에 줄 단위로 바뀌었다.
//              전에는 배열 하나를 통째로 덮어써서 기기끼리 부딪혔다
//
...
export function useFavs() {
  ...
  const onAccount = !!user && (status === 'ok' || status === 'pending');
  ...
  const toggle = useCallback((dex: number, from = '') => {
    ...
    if (!onAccount || !user) { toggleDevice(number); return; }
    ...
    const work = on ? serverApi.addFavorite(number) : serverApi.removeFavorite(number);
    void work.catch(() => {
  ...
  }, [enabled, onAccount, user, account, setAuth, toggleDevice]);
