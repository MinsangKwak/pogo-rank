...
//    등급 기준은 목록을 만드는 쪽이 정한다 — 2026-09-14 v3.30.0 부터 D-MAX 티어표는
//    **전 종 기준 절대 등급**(value_build.py absolute_tier)이라 탭을 옮겨도 글자가 그대로다.
//    PvE 일반은 아직 그 목록 안의 상대 등급이다. 빈 티어는 머리글까지 건너뛴다.
//
...
function renderPveEasy() {
  ...
  $note.textContent = '구하기 쉬운 일반 개체만 모은 레이드 티어표 (자체 계산). 속성 탭은 그 속성 포켓몬만 표시. 점수는 같은 속성 최강 어태커(전설·메가 포함) 대비 %, 티어는 목록 안 상대 등급. 포켓몬을 누르면 상세 정보가 열려요.';
}
