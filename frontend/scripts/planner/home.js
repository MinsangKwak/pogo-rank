...
// 내 포켓몬은 탭 줄, 도감 모드는 헤더 배지, 로그인은 ☰ 메뉴 계정 카드가 각각 유일한 진입점이다 (화면 하나에 버튼 하나).
...
function renderPlanHome() {
  ...
  const intro = el('div', { class: 'plan__card plan__intro' },
    el('p', { class: 'plan__title' }, '🌱 플래너 — 내 개체를 어떻게 키울까'),
    el('p', { class: 'plan__desc' }, '도감이 "뭐가 세나"에 답한다면, 플래너는 "내가 가진 이 개체를 지금 키워도 되나, 다음에 뭘 하나"에 답합니다. 위 탭의 🎒 내 포켓몬에서 개체를 레벨·개체값·기술 단위로 저장하면 같은 종끼리 비교할 수 있어요. 도감 상세 팝업의 ➕ 로도 바로 저장됩니다.'));

  let summary;
  if (!authEnabled()) {
    summary = el('p', { class: 'plan__desc' }, '이 빌드는 로그인 기능이 꺼져 있어 저장이 안 됩니다. 계산·조회는 할 수 있어요.');
  } else if (!loggedIn) {
    summary = el('p', { class: 'plan__desc' }, AUTH.status === 'pending'
      ? '⏳ 승인 대기 중 — 승인되면 개체를 계정에 저장하고 기기 간에 동기화합니다.'
      : '☰ 메뉴 맨 위 "👤 마이페이지" 에서 로그인하면 개체를 계정에 저장하고 어느 기기에서든 같은 목록을 봅니다. 로그인 없이도 CP 계산은 해 볼 수 있어요.');
  } else {
    summary = el('p', { class: 'plan__desc' }, mons.length
      ? `육성 중 ${countBy('육성 중')} · 완료 ${countBy('완료')} · 교환 후보 ${countBy('교환 후보')}`
      : '아직 저장한 개체가 없어요. 🎒 내 포켓몬 탭의 ➕ 개체 추가, 또는 도감 상세 팝업의 ➕ 로 시작하세요.');
  }
  const collectionCard = el('div', { class: 'plan__card' },
    el('p', { class: 'plan__title' }, loggedIn ? `🎒 내 포켓몬 ${mons.length}마리` : '🎒 내 포켓몬'),
    summary);
  const roadmap = el('p', { class: 'detail__foot plan__roadmap' }, '다음에 붙을 것: 육성 판단 카드(키울 가치·다음 행동) · 목표 자원 계산기 · 게임 검색식 생성기 · 보유 개체 기반 파티 · 내 목표 × 일정 연결');
