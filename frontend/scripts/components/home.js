// 서비스 진입 화면. 기능의 주 진입점을 한 화면에 모은다.
function renderServiceHome() {
  const features = [
    ...
    ['05', '레이드 · PvE', '추천 딜러와 솔플 계산기', '#/rank/pve', '⚔'],
  ...
  const grid = el('div', { class: 'service-grid' }, ...features.map(([number, title, desc, route, icon]) =>
    el('a', { class: 'service-tile', href: route },
      el('div', { class: 'service-tile-top' }, el('span', { class: 'service-icon', 'aria-hidden': 'true' }, icon), el('span', { class: 'service-number' }, number)),
      el('strong', {}, title), el('span', { class: 'service-desc' }, desc), el('span', { class: 'service-arrow', 'aria-hidden': 'true' }, '↗'))));
  $content.append(
    el('section', { class: 'service-intro' }, el('span', { class: 'service-eyebrow' }, 'YOUR POKÉMON COMPANION'),
      el('h2', {}, '오늘의 모험,', el('br'), '여기서 준비하세요.'),
      el('p', {}, '찾고, 비교하고, 키우는 즐거움. 필요한 기능으로 바로 시작해요.')),
    el('section', { class: 'service-features', 'aria-label': '서비스 기능' },
      el('div', { class: 'service-section-title' }, el('h3', {}, '무엇을 해볼까요?'), el('span', {}, 'EXPLORE')), grid));
