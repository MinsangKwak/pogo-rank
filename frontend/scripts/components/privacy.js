...
function renderPrivacyPage() {
  ...
  return el('div', { class: 'page-body' },
    p('POGO PLAN(이하 "서비스")는 Google 로그인으로 승인된 친구에게만 즐겨찾기 저장 기능을 제공합니다. 이 페이지는 그 과정에서 어떤 개인정보를 수집·보관하는지 설명합니다.'),
    el('p', { class: 'd-foot' }, '시행일 2026-09-04 · 개정 2026-09-06 (이용 통계에 계정 식별자 연결) · 2026-09-07 (플래너 개체 정보 저장 항목 추가). 서비스는 개인이 친구들을 위해 무료로 운영하며 상업적 목적이 없습니다.'),
    ...
    sec('수집하는 개인정보',
      ...
      ul(
        ...
        '🌱 플래너 "내 포켓몬"에 저장한 개체 정보(종·폼, 섀도우 여부, 레벨, 개체값, 기술, 상태, 메모) — 승인된 사용자만. 게임 안의 정보라 개인정보는 아니지만 계정에 저장되므로 명시합니다 (2026-09-07 추가)',
    ...
    sec('수집 목적과 보관 위치',
      ...
      ul(
        ...
        '즐겨찾기·내 포켓몬 동기화 — 어느 기기에서 로그인해도 같은 즐겨찾기와 개체 목록이 보이도록')),
    ...
    sec('쿠키·로컬 저장소',
      p('서비스가 직접 쿠키를 사용하지는 않습니다. 브라우저 로컬 저장소(localStorage)에는 "패치노트 읽음 여부", "도감 화면 열 수", "마지막으로 보던 탭·모드" 같은 개인정보와 무관한 설정만 저장합니다. Google 로그인·Firebase SDK가 자체적으로 세션 유지를 위한 저장소를 사용할 수 있습니다.')),
