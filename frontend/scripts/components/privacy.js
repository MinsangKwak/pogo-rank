...
function renderPrivacyPage() {
  ...
  return el('div', { class: 'page__body' },
    ...
    sec('2. 자동으로 수집되는 항목',
      ul(
        '방문 통계(Google Analytics 4) — **기본으로 켜져 있습니다.** 클릭한 탭 이름·포켓몬 이름·상성 검색 타입 등 이용 패턴과 기기·브라우저 종류를 기록하며, 로그인한 경우 Firebase uid 와 승인 상태(ok/pending)를 GA 의 User-ID·사용자 속성으로 보냅니다. 이메일·이름·사진은 보내지 않습니다. 광고 목적의 수집(ad_storage 등)은 꺼 두었습니다. **끄려면** 첫 방문 배너의 "통계 끄기" 또는 ☰ 메뉴 → 통계·저장소 설정에서 "통계 끄기" 를 고르면 되고, 그때부터 통계 스크립트를 아예 불러오지 않습니다',
