...
(async () => {
  ...
  for (const [w, h, label] of [[1440, 900, 'PC'], [390, 844, '모바일']]) {
    ...
    // 도감·즐겨찾기는 PC 에서만 카드가 기본이다 (wideCards, dom.js).
    ...
    for (const [hash, wantsGrid] of [['#/dex', wide], ['#/raids', true], ['#/favs', wide]]) {
      ...
      ok(`${label} ${hash} 카드 기본값 ${wantsGrid}`, grid === wantsGrid, String(grid));
    }
  ...
  // 함께 잃어버리던 버그가 있었다 — 첫 로딩만으로는 안 드러나고 왕복해야 나온다
  {
    ...
    for (const [w, wLabel] of [[390, '모바일'], [1440, 'PC 복귀'], [390, '모바일 재방문'], [1440, 'PC 재복귀']]) {
      ...
      ok(`폭 왕복 ${wLabel}(${w}) 이동 목록 10개·한 벌`, s.nav === 10 && s.navMenus === 1, JSON.stringify(s));
