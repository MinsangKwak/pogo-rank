...
suite(async () => {
  ...
  await page.locator('#screen-tabs .seg button:has-text("리틀")').click();
  ...
  await page.locator('#screen-tabs .seg button:has-text("슈퍼")').click();
  await page.waitForTimeout(600);
  ...
  // 위에서 '슈퍼' 를 골라도 아래 상위 10 은 '리틀' 인 일이 생겼다
  {
    ...
    await p3.locator('#screen-tabs .seg button:has-text("하이퍼")').click();
    ...
    await p3.locator('#screen-tabs .seg button:has-text("마스터")').click();
    await p3.waitForTimeout(600);
