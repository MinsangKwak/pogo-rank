...
| `backup_ack` | "이 줄어듦은 사고가 아니다" 를 **사람이** 못 박는 자리 (v4.8.5) | 계속 |
...
**③④의 잣대는 여태까지의 가장 큰 수다 — 창이 없다** (v4.8.4·v4.8.5).
...
**기계가 스스로 잊지 못하게 하고, 잊는 일은 사람이 한다.**
...
**못 박은 뒤에 되살아났으면 잣대도 따라 올라간다** (v4.8.5). 안 그러면 잣대가 낮은 채로 굳는다 —
40 으로 못 박고 100 으로 돌아온 뒤 다시 40 이 되면 그건 **새 사고**인데 옛 못이 덮어 버린다.
그래서 못 박은 수와 **`acked_at` 이후에 본 가장 큰 수** 중 큰 쪽을 쓴다.

**③은 ④가 아무것도 못 짚었을 때만 말한다.** 같은 사고를 두 번 외치지 않는다 —
