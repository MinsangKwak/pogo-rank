...
| `DROP_RATIO` · `LOOKBACK_RUNS` | 0.3 · 5 | ④ **컬렉션 하나**가 30% 넘게 줄었거나 아예 사라졌다 (v4.8.1) |
...
**④의 잣대는 바로 앞 판이 아니라 최근 `LOOKBACK_RUNS` 판의 가장 큰 수다** (v4.8.4).
앞 한 판만 보면 `100 → 0 → 0` 이 **두 번째 판에서 통과한다** — 견줄 앞이 이미 0 이고 총합도 더 안 줄어서다.
안 고쳐진 손실은 고쳐질 때까지 계속 보여야 한다. 되살아나면 그 판에서 바로 조용해진다.
