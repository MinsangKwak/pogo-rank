...
`git commit -s` 가 자동으로 붙입니다. 이 서명은 "내가 이 코드를 이 저장소의 조건(LICENSE)으로 기여할 권리가 있다"는 확인입니다.
...
기여한 코드는 저장소와 같은 조건(LICENSE)으로 이 저장소 안에서 쓰입니다 — **포크와 재배포는 허용하지 않습니다.** 기여는 포크가 아니라 이 저장소로 보내는 이슈·PR 로 합니다. 데이터·이미지는 NOTICE.md 의 범위를 따릅니다.
